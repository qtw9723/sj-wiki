...
## 🆕 🔥 경쟁 축 — **「접근성」은 이미 상품 카테고리다. 해자는 결정론 안전장치 체인 쪽에 있다** (2026-09-09 · W37 6차 감사)

> 🧠 이 절은 **판단 영역**이다. 📄 근거는 [[AI-주간-소식-2026-W37]]에 한 주간 쌓인 국내 상용 발표이고, 위 §주요 기능이 *"무엇을 만들었는가"* 라면 이 절은 **"그중 무엇이 방어 가능한가"** 를 적는다.

### 🚨 📄 이번 주에 「접근성」을 파는 제품이 셋 나왔다

| ID | 📄 제품 | 📄 파는 문장 |
|---|---|---|
| (KR-W37-35) | **AXPort** (농협·웹케시 경영진 AI 아카데미) | *"**개발자가 아닌 실무자**도 Claude Code 기반으로 기업 보안 요건을 충족하며 업무 자동화 구현"* |
| 🔥 (KR-W37-53) | **와이즈넛 WISE Agent Labs** | *"**노코드 드래그 앤 드롭**으로 워크플로우 구성. **실무 담당자도 에이전트를 직접 구축**. 멀티 LLM·RAG 지원"* — 🚨 **조달청 디지털서비스몰 등록** |
| (KR-W37-66) | **토마토시스템 엑스빌더6 아이젠** | *"요구사항·개발 의도를 이해해 **기획·설계부터 UI 구현·코드 생성까지**"* |

- 🔥 🚨 **와이즈넛 건의 알맹이는 노코드가 아니라 「조달청 등록」이다.** 🧠 *조달청 디지털서비스몰 등록* = **공공기관이 별도 입찰 절차 없이 바로 구매할 수 있는 경로 확보**. 📄 이건 [[AI-주간-소식-2026-W37]] (KR-W37-29) GLM 코딩 플랜이 **알리바바 티몰**에 스토어를 연 것과 **같은 구조의 사건**이다 — 🧠 *"에이전트 도구가 원래 없던 구매 경로로 내려왔다"* 가 이번 주에만 두 건.
- ✅ 🧠 **그래서 경쟁 축을 *"실무자도 쓸 수 있다"* 에 두면 안 된다.** 접근성은 이미 카테고리이고, 그중 하나는 조달 경로까지 뚫었다.

### 🔥 ✅ 실제 해자는 §아키텍처에 이미 있다 — **구조 정합성을 사람이 아니라 코드가 보증한다**

- 📄 위 §아키텍처: *"LLM은 설계만, **구조 정합성은 코드가 보장**하는 하이브리드(LLM+결정론)"*. 📄 위 §주요 기능의 **결정론적 안전장치 체인 5종** — Stage2.9 루프 탈출변수 정합화 → 2.95 반복수집 루프 래핑 → 3 검증 재시도 루프 래핑 → 3.5 무의미 컨디션 노드 제거 → 3.6 루프 누적 배열 자동 초기화.
- 🧠 **드래그앤드롭 노코드 빌더와의 결정적 차이가 여기다**: 노코드는 **사람이 그린 워크플로우를 그대로 실행**하므로 *"이 분기가 도달 불가인지 / 탈출 변수가 세팅되긴 하는지"* 를 **사람이 보증**한다. CogInsight는 `validate/repair`가 **본문 도달성·exitVar set 존재를 코드로 검증**한다.
- ✅ 🧠 **이 문장이 방어하기 더 쉽다**: *"실무자도 쓸 수 있다"* 는 UI 투자로 따라잡히지만, *"틀린 시나리오가 산출물로 나가지 않는다"* 는 **도메인 규칙을 알아야 만들 수 있다**([[내-프로필]] §포지셔닝 축의 *복제 불가 자산*).
- 🔥 📄 **시장도 같은 축에 돈을 붙이기 시작했다**: (KR-W37-54) 신한카드가 *"생성형 AI의 **할루시네이션**을 극복하고 금융 상품 추천의 **신뢰성**을 극대화"* 하려고 중기부 R&D에 착수했다 · (KR-W37-44) 에이아이매틱스는 도메인 특화로 *"인식률 99.2%"* 를 판다. 🧠 **정확도·신뢰성이 별도 예산 항목이 됐다** = 결정론 체인이 기능이 아니라 **판매 포인트**가 될 수 있다.
- ⚠ 📄 와이즈넛 빌더의 산출물 형태(워크플로우 정의까지인지, 실제 봇 설정 파일까지인지)는 raw에 **없음** — 기능 대 기능 직접 비교는 근거 부족. 🧠 위 비교는 *"노코드 일반"* 대비 구조 논지이며 특정 제품 열위 주장 아님.

### 🆕 🔥 해자의 **다음 칸은 「모니터링」이다** — 상용 하이퍼오토메이션이 세 칸을 한 줄로 판다 (2026-09-10 · W37 8차 감사)

📄 근거: [[AI-주간-소식-2026-W37]] (KR-W37-75) · `raw/ai-digest/naver-2026-09-10.md` § AI자동화 · it-b.co.kr

> 📄 *"**삼성SDS Brity Automation**: 기존 RPA의 단순·반복 업무 자동화를 넘어 AI와 Agent 기술을 결합한 **하이퍼오토메이션 플랫폼**. **자연어 기반 설계·실행·모니터링** 지원."*

- 📄 09-08의 (KR-W37-39) **브리티웍스**(*"업무 목표를 입력하면 자료조사→보고서→메일→일정까지 스스로 수행"*)와 **같은 Brity 제품군**이다 = 이틀 간격으로 같은 라인이 두 각도로 들어왔다.
- 🔥 🧠 **문장이 이 프로젝트의 문장과 겹친다**: *"**자연어 기반 설계·실행**"* = CogInsight의 *"자연어 요구사항 → LLM이 시나리오 구조 설계 → 결정론 코드가 정합성 보증"*. ✅ 그런데 **세 번째 칸(모니터링)이 여기엔 없다.**
- ✅ **그래서 위 §해자 논지에 항목이 하나 붙는다**: 6차 결론은 *"접근성은 이미 카테고리 → 해자는 결정론 안전장치 체인"* 이었다. 🧠 [[에이전트-자동화-도구]]가 반복해 적은 *"값은 **관측·증거** 칸에 남는다"* 와 붙이면 — **생성한 시나리오가 실제 운영에서 어떻게 동작했는지 되짚는 층**이 결정론 체인 다음의 자연스러운 칸이다. 🧠 그리고 그 층은 결정론 체인과 **같은 근육**이다: 체인이 *나가기 전에* 틀린 걸 막는다면, 모니터링은 *나간 뒤에* 무엇이 틀렸는지 남긴다.
- ⏳ **현재 미구현 · 로드맵 후보로만 기록**(⚠ 이 회차에서 코드·설계 변경 없음).
- ⚠ 📄 Brity Automation의 모니터링이 무엇을 관측하는지(실행 로그·비용·품질)는 raw에 **없음** — 기능 대 기능 비교 근거 아님.

### 🆕 🔥 ⭐⭐⭐ 이 프로젝트의 **전제**가 다른 산업에서 같은 문장으로 나왔다 — *"병목은 하드웨어가 아니라 프로그래밍"* (2026-09-11 · W37 9차 감사)

📄 근거: [[AI-주간-소식-2026-W37]] (KR-W37-97) · `raw/ai-digest/naver-2026-09-11.md` § AI자동화 · [naver/030](https://n.news.naver.com/mnews/article/030/0003465801?sid=101)

> 📄 *"자이스웍스-토론토대 **행동 모방 AI 로봇 제어** 공동연구. **제조 현장 자동화의 병목이 하드웨어가 아닌 프로그래밍에 있다는 진단** 하에, **AI 기반 제어 생성**으로 접근."*

- 🔥 🧠 **문장 구조가 이 프로젝트의 전제와 동일하다**:

  | | 📄 자이스웍스(제조) | 🧠 CogInsight(챗봇) |
  |---|---|---|
  | 이미 있는 것 | 로봇 하드웨어 | 사내 NLP 챗봇 솔루션 |
  | **병목** | **로봇을 프로그래밍하는 일** | **시나리오를 사람이 손으로 짜는 일** |
  | 처방 | AI 기반 **제어 생성** | LLM 기반 **시나리오 생성** |

- ✅ 🧠 **유비가 아니라 같은 문제 유형의 두 인스턴스다.** 🧠 그래서 포트폴리오·설명 자리에서 쓸 수 있는 문장이 하나 생긴다: *"설비가 아니라 **설비를 지시하는 일**이 병목이라는 진단은 제조 자동화에서도 같은 해에 같은 말로 나왔다"* — 도메인 지식 없는 심사자에게 **문제 정의의 일반성**을 설명하는 가장 짧은 경로다.
- 📄 그리고 이건 (KR-W37-05) 앤트로픽-삼성(*"기존 프로세스 단순 자동화는 실패 → **프로세스 근본 재설계**"*, 09-02)의 **제조 판본**이다 = 같은 주에 두 산업에서 같은 진단.
- 🧠 같은 날 (KR-W37-110)(*반도체 장비가 **이상 원인을 판단**하고 필요 동작을 수행*)이 그 처방의 **장비 쪽 판본**이다.
- ⚠ 📄 *"행동 모방"* 의 학습 방식·데이터 규모는 raw에 **없음** — 기술 비교 근거 아님.

### 🆕 ⭐⭐ 「모니터링 칸」의 **내부 구조가 보였다** — 한 기능이 아니라 **다섯 역할**이다 (2026-09-11 · W37 9차 감사)

📄 근거: [[AI-주간-소식-2026-W37]] (KR-W37-114) · `naver-2026-09-11.md` § 에이전트AI (기사 2건 묶음)

> 📄 *"마켓핏랩 × 믹스패널 **MXP Seoul 2026** — **RCA·온보딩·KPI 모니터링·대시보드·실험**에 특화된 AI 에이전트들이 프로덕트 데이터를 **상시 모니터링하고 이상 징후를 자동 포착**."*

- 🧠 위 §(2026-09-10) 항목은 (KR-W37-75) Brity의 *"설계·실행·**모니터링**"* 한 줄에서 **이 프로젝트에 비어 있는 칸**을 찾았다. 오늘 건은 **그 칸이 어떻게 쪼개지는지**를 보여준다 — 📄 모니터링이 단일 기능이 아니라 **역할별 에이전트 묶음**이다.
- ✅ 🧠 **그래서 로드맵 후보의 모양이 바뀐다**: *"모니터링을 붙인다"* → **"다섯 역할 중 어느 것부터 붙일지 고른다"**. 🧠 CogInsight에 가장 가까운 칸은 **RCA(생성된 시나리오가 실봇에서 왜 실패했는가)** 이고, 이건 §주요 기능의 **결정론 안전장치 체인·회귀 fixture와 같은 데이터**를 쓴다 = 새 계측이 아니라 **기존 산출물의 사후 활용**이다.
- 🧠 그리고 이건 §해자 논지와 대칭이다: 📄 체인이 *나가기 전에* 막고, RCA는 *나간 뒤에* 왜 틀렸는지 남긴다 — **같은 근육**.
- ⚠ 📄 마켓핏랩 제품의 실제 구현(에이전트가 진짜 분리돼 있는지, 프롬프트 프리셋인지)은 raw에 **없음** — 아키텍처 근거 아님.

### 🆕 ⭐ *"정확성·신뢰성이 별도 예산 항목"* 근거 **3건째 — 이번엔 공공 발주다** (2026-09-11)

- 📄 (KR-W37-102) **강원도-KOTRA** — *"해외무역관 정보·수출통계 등 **검증된 데이터 20종에 생성형 AI를 결합**. 기업들의 자체 시장조사 비용 부담 · **AI 정보 신뢰성 검증 어려움 해소** 목적."*
- ✅ 🧠 위 §해자 절이 근거로 든 (KR-W37-54) 신한카드(*할루시네이션 극복·신뢰성 극대화* R&D) · (KR-W37-44) 에이아이매틱스(*인식률 99.2%*)에 이은 **세 번째**이고, 앞선 둘은 민간 R&D인데 **이건 지자체 협약 = 공공 예산**이다.
- 🔥 🧠 **방법까지 같다**: LLM 자유 생성이 아니라 **「검증된 데이터 20종」이라는 결정론적 기반 위에 생성을 얹는다** = 이 프로젝트의 *"LLM은 설계만, 구조 정합성은 코드가 보장"* 과 같은 형태. ✅ *"신뢰성을 어떻게 파는가"* 의 레퍼런스 문구로 인용 가능.

### 🧠 작업 표면이 **캔버스로 수렴하고 있다** (KR-W37-81 · 관찰)

- 📄 (KR-W37-81) 유토파이 **'파이 2.0'** — *"**크리에이티브 에이전트와 캔버스 기반 워크스페이스**, 이미지·영상 생성 모델 결합."* 📄 (KR-W37-53) 와이즈넛 — *"**노코드 드래그 앤 드롭**으로 워크플로우 구성."*
- 🧠 도메인이 영상 제작 / 업무 워크플로우 / 챗봇 시나리오로 다 다른데 **작업 표면의 형태가 같다** — *공간 위의 노드·흐름 + 그 위에서 일하는 에이전트*. ✅ CogInsight가 만드는 **시나리오 흐름도**가 정확히 그 구조다.
- ⚠ 🧠 근거는 제품 3건뿐 — *"업계가 캔버스로 표준화됐다"* 는 주장이 아니라 **UI 선택의 방향성 관찰**이다.

### ⚠ 🧠 함께 붙는 어휘 리스크 (KR-W37-58)
...
### 🆕 🔥 ⭐⭐ 비용 축 갱신 — **단가 갈래가 사흘 만에 스스로 무너졌다** (2026-09-23 · W39 6차 감사)

- 📄 **(KR-W39-69)** — **OpenAI GPT-6 Sol·Luna**와 **Anthropic Claude Opus 5.5** 동시 공개, OpenAI는 *"GPT-6 Sol과 Luna의 **API 가격을 GPT-5.6 프로모션 가격보다 50% 인하**"*([토큰포스트](https://www.tokenpost.kr/news/breaking/412425)). 📄 Sol은 전문 업무용, Luna는 속도 최적화.
- 🔥 🧠 **위 절이 두 갈래를 비교하고 *"후자(호출 횟수)가 이 프로젝트에 더 맞는다"* 고 판단한 지 사흘 만에, 앞 갈래가 자기 근거를 무효화했다**: 📄 09-20 Jev *건당 0.0004달러* → 09-23 **경쟁 라인 50% 인하**. ✅ 🚨 **단가 갈래의 문제는 「비싸다」가 아니라 「고른 순간 낡는다」** 는 것이다 — 모델을 골라 박아 두면 그 결정은 **주 단위로 열화**된다.
- ✅ 🔥 **그래서 설계 처방이 한 단계 구체화된다**: 이 파이프라인에서 오래 남는 결정은 *어떤 모델을 쓰는가*가 아니라 ***Stage별 요구사항 → 모델*** 매핑을 **분리 레이어로 뽑아 뒀는가**이다. 🧠 즉 지금 해야 할 작업은 *더 싼 모델로 갈아타기*가 아니라 **모델명이 하드코딩된 지점을 찾아 라우팅 설정으로 빼기**다 — 목적이 비용 절감이 아니라 **변경 흡수**라는 점이 다르다(⚠ 현재 하드코딩 지점 목록은 이 페이지에 **없음** — 다음 작업 후보).
- ✅ 🧠 **포트폴리오 문장으로도 이쪽이 낫다**: *"싼 모델을 골랐다"* 는 사흘이면 틀리지만, *"모델이 바뀔 것을 전제로 경계를 그었다"* 는 [[내-프로필]] §포지셔닝 축의 *복제 불가한 판단* 쪽에 남는다.
- ⚠ 📄 **Claude Opus 5.5의 가격·벤치마크는 raw에 없고**(인하 발표는 OpenAI 쪽이다), 📄 *"50% 인하"* 의 기준선도 **정가가 아니라 GPT-5.6 「프로모션 가격」** 이다 — 🚨 절대 단가 비교로 쓰지 않는다. 🔗 [[Claude-Code-업데이트-동향]] §②
- 🆕 ✅ ⭐⭐ **Opus 5.5 독립 재확인 — *속도 30%↑, 비용 40%↓*** (2026-09-24 · 📄 [[AI-주간-소식-2026-W39]] (KR-W39-92·93) · 7차 자동):
  - 📄 09-24 복수 소스에서 **독립 재확인**: *"Claude Opus 5.5 — 기존 모델 대비 **속도 30% 향상, 비용 40% 절감**, Fable 5.1 수준 성능 유지"*. 📄 같은 날 **Zeb 초경량 모델**(KR-W39-102) — *"**1B 파라미터 미만**으로 에지 디바이스 추론, 일반 벤치마크 상위권 주장"*.
  - 🔥 🧠 **09-23의 ⚠ 주의(「raw에 가격 없음」)가 09-24에서 해소됐다**: 📄 속도·비용 수치가 독립 소스에서 재확인됐으므로 이 파이프라인에 직접 적용 가능하다. ✅ **처방이 두 단계로 업데이트된다**:
    - 📄 **모델 라우팅 레이어 설계 재확인**: Opus 5.5 도입 시 *Stage1 판단 전용*(비용 40%↓) + *Stage2 결정론 전개*(모델 무관) 분리가 **비용 구조를 직접 바꾼다** — 위 §비용 축 처방(*"모델명이 하드코딩된 지점을 라우팅 설정으로 빼기"*)의 1순위 실행 대상이 됐다.
    - 📄 **Zeb = Stage1 판단 전용 저비용 선택지 후보**: 1B급 에지 모델이 Stage1(단순 분기 판정)을 맡고 Opus 5.5가 Stage2(복합 시나리오 생성)를 맡는 **2-tier 구조** 후보 추가. ⚠ 다만 Zeb 벤치마크·텍스트 생성 품질은 raw에 **없음** — 에지 이미지·분류 모델 기반 주장일 가능성 있음, 검증 전 하드코딩 금지.
  - ⚠ 📄 **속도·비용 수치의 기준선(기존 어느 모델 대비인지)은 raw에 없음** — 🚨 *"Opus 5.5가 모든 용도에서 40% 싸다"* 로 일반화하지 않는다.

### 🆕 ⭐⭐ 구조 대조 — **상품 쪽 AX 단계표에는 이 파이프라인에 없는 0단계가 있다** (2026-09-23 · (KR-W39-62))

- 📄 **플래티어** 금융권 대상 **4단계 AX 전략**: *① 지식 자산화 → ② 업무 실행 → ③ 협업 자동화 → ④ 자율 진화*. 자체 에이전틱 AI 플랫폼 **엑스젠(XGEN)** 의 **하이브리드 RAG**와 **자연어 기반 UI** 공개([팍스경제TV](http://www.paxetv.com/news/articleView.html?idxno=282630)).
- 🔥 🧠 **①을 그냥 지나치면 안 된다**: 이 파이프라인은 *Stage1 설계 → Stage2 결정론 전개 → 안전장치 체인* 으로 **생성부터 시작**하는데, 📄 상품 쪽은 **지식 자산화를 맨 앞 단계로 따로 센다**. ✅ 🧠 이 프로젝트에서 그에 해당하는 것은 **시나리오 레퍼런스 라이브러리**(v0.3.0 로드맵)인데, 현재 서술은 그걸 *기능*으로 두고 **단계로 두지 않는다** — 📄 위 §해자 절이 *"결정론 안전장치 체인"* 을 해자로 지목한 것과 나란히, **축적된 시나리오 자산**이 두 번째 해자 후보라는 뜻.
- ⚖ ⚠ 🚨 **다만 이건 판매 자료의 단계표다** — 📄 성과 수치·도입사·④*자율 진화*의 실제 내용은 raw에 **없다**. 🧠 4단계 서사는 *다음 계약을 파는 형태*이기도 하므로 **구조만 참고**하고 단계 이름을 그대로 빌려 쓰지 않는다.

### 🆕 ⭐ 시나리오 설계 — **페르소나는 편의 기능이 아니라 「지속 사용」 변수다** (2026-09-23 · (KR-W39-73))

- 📄 **트라이앵글 커넥트** 특허 출원 — CHI 2024 연구 인용: *"**대형언어모델(LLM) 기반 대화형 에이전트의 페르소나를 사용자가 직접 설정할 수 있을 때 정서적 유대감과 대화 지속 의향이 눈에 띄게 향상**"*([네이버뉴스/020](https://n.news.naver.com/mnews/article/020/0003750341?sid=101)).
- 🧠 **이 프로젝트 관점에서 값은 「효과」가 아니라 「누가 설정하는가」다**: 📄 연구가 말하는 조건은 *사용자가 **직접** 설정할 수 있을 때*다 — ✅ 시나리오 안에 페르소나 문구를 박아 넣는 구조와, **페르소나를 별도 레이어로 빼서 바꿀 수 있게 두는 구조**는 산출물이 아니라 **결과 지표**가 달라진다는 뜻.
- ✅ 🧠 위 §비용 축의 *모델 라우팅 분리*와 **같은 형태의 처방**이다(바뀔 것을 분리한다). ⚠ 📄 표본 수·효과 크기·특허 청구범위는 raw에 **없음**(CHI 2024 인용 문장만).
- 🧠 덧: 📄 **(KR-W39-78) 퀄컴 2나노 스냅드래곤**(멀티에이전트 기반 적응형 칩)은 (KR-W39-20+22) 카이스트 · (KR-W39-36) SKT · (KR-W39-48) 삼성 NPU 분산에 이은 **Stage 분리 원칙의 네 번째 독립 방증**이다 — 🚨 다만 전부 *하드웨어·통신사 전략*이고 **텍스트 생성 파이프라인에서의 검증은 아니다**(구조적 유비까지만).

---

## 진행 현황
> 📄 **(직전) v0.9.0**(기존 챗봇 업로드 후 수정 + 자산 역파생, 2026-08-28 — PR #120 main `99e8705`, tag v0.9.0). Cogi가 만들지 않은 **솔루션 챗봇 JSON을 올려** 다이어그램으로 보고 편집하고 다시 내려받는다(`/upload`). 올린 봇에서 **NLU·ESD를 역파생**한다(신규 함수 `derive-assets`). 핵심 불변식은 **왕복 무손실** — `generated_json`이 곧 원본이고 편집은 델타 패치로만. 회귀 3종 × 실제 봇 4 fixture로 고정. 노드 타입 레지스트리(화이트리스트) — 편집 8종/잠김 13종, 잠금 사유를 코드에 분류. 잠긴 노드도 내용은 읽기 좋게 표시되고 이름은 편집된다. 직전 v0.8.1(NLU 기준 조건 설정, PR #119 `91628fb`).
> 📄 **현재 프로덕션 = v0.10.1**(부가 도구: 데이터 비교, 2026-09-15 — PR #127 머지 main `2a92db1`, tag v0.10.1 + GitHub Release). CSV 두 개를 넣으면 무엇이 추가·삭제·변경됐는지 보여주는 화면(`/compare`). **프론트 전용** — DB·엣지함수·마이그레이션 무접촉이라 배포는 Vercel + 사내 온프렘뿐. 순수 모듈 6개(`src/lib/compare/`)에 테스트 75개, 저장소 전체 1004개 통과. ⚠ **기존 엑셀 파서를 재사용하지 않았다** — `normalizeSheet`가 완전 빈 열을 제거·셀을 트림해서 엔티티 CSV 13칸 고정 규격(동의어가 위치로 들어감)을 망가뜨린다(파일마다 살아남는 컬럼 수가 달라져 비교 자체가 어긋난다). ⚠ 리뷰가 **"오류 없이 틀린 답을 보이는" 경로 4건**을 잡아 전부 수정(가장 큰 것: after 쪽 동의어가 더 많으면 늘어난 동의어가 조용히 사라지고 "변경 없음"으로 표시되던 것 — `setIndexes`를 before에서만 뽑은 탓). 브라우저 실측 8건 통과. Vercel·사내 온프렘·공개 개요 페이지·아티팩트 모두 반영 완료(개요 페이지의 통계 카드가 v0.5.1에 멈춰 있던 드리프트도 함께 정정).
> 📄 **(직전) v0.10.0**(생성 데이터 관리 & 자산 모듈화, 2026-09-11 — PR #126 머지 main `4ee0367`, tag v0.10.0 + GitHub Release). 항목 6개 전부 완료: 번들 계약·중복 키 정리·NLP 만들기 도구(`/nlp`)·모듈 이동·**자산 라이브러리**·결과 보존 정책. 핵심은 **부산물이던 NLP·ESD·API를 독립 자산으로** 올린 것 — 보관한 결과에서 시나리오 하나를 자산째로 등록하고, 여러 개를 골라 조립해 새 결과로 저장한다(조립물도 기존 다운로드 그대로). 신규 테이블 `coginsight_scenario_assets`(+dev 미러·RLS), 신규 함수 `scenario-assets`(+`-dev`). 용어 **NLU→NLP 전면 리네임**(PR #125). 마이그레이션 1건 적용 · 영향 함수 12종 배포 · Vercel·사내 온프렘·공개 개요 페이지·아티팩트 모두 반영 완료.
> 🔶 **현재 개발 = 없음.** 다음은 v0.11.0(로직 정합화·부채 정리) 또는 v0.12.0(UI 전면 정리) 착수 예정. ⚠ v0.11.0 후보로 이월된 것: 자산 삭제 UI·관리자 삭제 경로(서버측 관리자 토큰 검증이 저장소에 없다), 동명 시나리오 인텐트 겹침, `useResults` 두 벌 통합, few-shot 주입(B단계).
...
| 버전 | 날짜 | 기준선 | 핵심 내용 |
|---|---|---|---|
| **v0.10.1** | 2026-09-15 | main `2a92db1` · PR #127 · tag v0.10.1 · **프론트 전용**(DB·엣지함수 무변경) | **부가 도구: 데이터 비교** — CSV 두 개의 차이를 보는 화면(`/compare`). 헤더만 보고 인텐트·엔티티·ESD CSV를 알아보고, 모르는 CSV는 키 컬럼을 고른다. 인텐트는 키가 `intent`+`example`이라 '변경'이 원리상 없고(화면에서 인텐트별로 묶어 보임), 엔티티 동의어는 **집합으로** 비교해 순서 변경을 차이로 보지 않는다. 기본은 **원문 그대로** 비교(공백·대소문자 완화는 체크박스, 기본 꺼짐). 차이만 담은 CSV 내보내기(BOM 있음). ⚠ 기존 `parseExcelFile`/`normalizeSheet` 재사용 금지 — 빈 열 제거·셀 트림이 엔티티 13칸 규격을 깨뜨린다. 순수 모듈 6개·테스트 75개(전체 1004). ⚠ 브리프가 준 테스트 3개가 **동어반복**이라 강화했다 — 개수는 증거가 아니다. |
| **v0.10.0** | 2026-09-11 | main `4ee0367` · PR #126(항목5)·#124(항목6)·#125(NLP 리네임) · tag v0.10.0 · 신규 테이블 1종 · 신규 함수 1종 | **생성 데이터 관리 & 자산 모듈화** — Added: 번들 계약(`contract.js`, 화면은 정규화된 `result.bundle`만 읽고 정적 검사가 강제) · `/nlp` NLP 만들기 도구 · **자산 라이브러리**(시나리오를 자산째로 등록 → 조립 → 새 결과 행으로 저장, 신규 테이블 `coginsight_scenario_assets`·함수 `scenario-assets`) · 결과 **보관 표시·필터**(DB 스키마 변경 없이 번들 키). Changed: 중복 개수 키 3종 제거·`tier*`/`stage*` 14종 정리, `deriveNlpData`·`intentName`을 `_shared/`로 이동, **NLU→NLP 전면 리네임**. ⚠ 인텐트 귀속은 **이름 기반**(위치 기반은 틀렸다 — 인텐트 배열은 설문 순서, 인덱스는 `main.then` 순서). ⚠ 조립물은 세 번째 결과 종류라 게이팅 전수 수정 + `derive-assets` 서버측 409 가드. dev 브라우저 실측 7항목 통과. |
...
| **v0.10.0** | 생성 데이터 관리 & 자산 모듈화 | ✅ 2026-09-11 릴리스 |
| **v0.10.1** | 부가 도구: 데이터 비교 | ✅ 2026-09-15 릴리스 |
...
## 의외의 연결점 (🧠 판단 영역)
- 🆕 🔥 ⭐⭐ **(2026-09-13 · W37 12차 감사) 「어떤 모델을 언제 쓸지」가 국내 대형 플랫폼에서 제품 층으로 올라왔다 — 라우팅은 기능이 아니라 층이다**:
  - 📄 [[AI-주간-소식-2026-W37]] (KR-W37-143) **KT 이음 인사이드 / 토큰팩토리** — *"**질문·상황에 따라 최적 모델을 선택**하는 멀티모델 전략"*, *"하나의 AI 모델만 고집하지 않고"*, *"NPU·멀티모달·솔트룩스 루시아 등을 결합한 라우팅 전략"*.
  - 🧠 **이 프로젝트에 걸리는 형태는 KT와 다르다.** KT는 *사용자 질문*이 들어올 때 고르는데, 여기서는 **파이프라인 스테이지가 이미 정해져 있다**(6단계). ✅ 즉 필요한 건 *동적 라우팅*이 아니라 **스테이지별 모델 고정 + 그 고정을 한 곳에서 바꾸는 지점** — 🔥 지금 `gpt-5.2` 단일 고정(§2026-07-09 v0.3.1)이므로, **라우팅을 만들기 전에 「모델 선택이 코드 어디에 박혀 있는가」를 한 지점으로 모으는 것**이 선행 작업이고 그 자체가 [[mailer]] §정책 리스크 4건째와 **같은 처방**이다(모델 교체 지점의 인터페이스 분리).
  - ✅ 🧠 **비용 관점이 라우팅의 실제 동기다**: 📄 (W36-EN3) 비용 공식 `호출수 × 토큰 × (1−캐시적중률)` 에 **모델 단가**라는 넷째 항이 붙는다 — 🧠 6단계 중 *조립·검증* 같은 결정론에 가까운 단계까지 플래그십을 쓸 이유가 없다. ⚠ 📄 KT의 라우팅 알고리즘·모델 목록·정확도 수치는 raw에 **없음** — 구조 참고까지만.
  - 🧠 **⚠ 그러나 KT를 경쟁 참고로 읽지는 않는다**: 📄 KT의 차별점은 *"검색 이후의 **실행**"* 이고 이 프로젝트의 산출물은 **실행 전의 시나리오 정의**다. ✅ 아래 (2026-09-05) *액션 슬롯* 항목이 그 접점이고, 라우팅은 그것과 별개 층이다.
- 🆕 ⭐⭐ **(2026-09-13 · W37 12차 감사) 입력 UX가 품질 변수라는 게 「Claude 사용법」 쪽에서도 같은 문장으로 나왔다**:
  - 📄 [[AI-주간-소식-2026-W37]] (KR-W37-146) — *"Claude를 잘 사용하는 것은 **AI가 모든 것을 판단하도록 맡기는 게 아니라, 자료·제약·형식을 분명히 지정하는 것**"*([dailyt](https://www.dailyt.co.kr/newsView/dlt202609120012), 2026-09-12).
  - 🔥 🧠 **이 프로젝트는 이미 그 구조다** — 지식소스(자료) · `solution_rules`(제약) · Dialog JSON 스키마(형식). ✅ 즉 오늘 건은 새 기능 제안이 아니라 **기존 설계의 정당화 문장**이고, 🧠 값은 거기서 한 칸 더 간다: *"사용자가 세 칸을 채우게 하는 UX"* 와 *"프롬프트 입력창 하나"* 가 산출물 품질에서 갈린다면, **세 칸을 비워 둔 채 생성했을 때 무엇이 나빠지는지를 측정 가능하게 만드는 것**이 [[프로젝트-포트폴리오]]에 쓸 수 있는 형태다.
  - 🧠 📄 (KR-W37-65) 대학 강의 *"AI가 최종 판단을 내리는 순간은 허용하지 않는 원칙"* 과 같은 결론의 **개인 사용법 판본**이고, [[내-프로필]] §포지셔닝 축(*판단·책임이 남는 자리*)의 도구 층 근거다.
...
- 🔥 ⭐⭐⭐ **🆕 (2026-09-18) *"평가는 유형별로 분해"* 가 이틀 만에 임베딩 층에서 재확인됐다 — 그리고 비용도 분해해야 한다** ([[AI-주간-소식-2026-W38]] (KR-W38-118)·(KR-W38-120) · 10차 감사):
  - 📄 **인천대 EMNLP 2026 채택 — *'LEVEE: 11개 대형 언어 모델에 대한 **벡터 인코딩 효과의 계층별 평가**'***([CS타임즈](https://www.cstimes.com/news/articleView.html?idxno=721159)). ✅ 8차가 📄 (KR-W38-90) 의료 AI *"전체 정확도를 넘어 **하위집단별** 성능 검증"* 에서 뽑아 이 페이지에 넣은 원칙과 **완전히 같은 형태**다 — 한쪽은 *데이터 집단*으로, 이쪽은 *모델 계층*으로 쪼갠다.
  - ✅ **적용안 ①(평가)**: RAG 검색 품질을 *"임베딩 모델을 X로 바꿨더니 좋아졌다"* 는 **단일 점수**로 판정하지 않는다 — **질의 유형별**(정의형 / 절차형 / 예외 처리형)로 나눠 본다. 🧠 단일 점수는 유형별 편차를 가리고, 그 편차가 바로 시나리오 품질이 갈리는 자리다. ⚠ 📄 LEVEE의 구체 결론(어느 계층이 유리한가)은 raw에 **없음**(제목만 보도).
  - 📄 **디노티시아 VDPU** — *"**생성형 AI 모델 실행은 GPU가, RAG와 AI 에이전트의 벡터 검색은 VDPU가 분담**. 4096차원 멀티모달 인덱스 구축 시 호스트 CPU 92%·메모리 73% 감소"*([ZDNet](https://zdnet.co.kr/view/?no=20260918084548)). 🧠 [[AI-주간-소식-2026-W34]] (KR94) 씨홀스 벡터DB를 낸 그 회사가 **한 달 만에 서버 폼팩터로 확장**했다.
  - ✅ **적용안 ②(비용)**: 🧠 함의는 *"하드웨어를 사라"* 가 아니라 **분담선 자체**다 — *생성(LLM 호출)* 과 *검색(인덱싱·질의)* 은 **비용 구조가 다른 별개 워크로드**인데, ⚠ 지금 이 제품은 둘을 같은 호출 경로 안에서 불러 **어느 쪽이 비싼지 분리 측정되지 않는다**. 📄 (W34-EN17) Multi-Vector(Late Interaction)가 *"모델 교체 없이 인덱싱 방식만"* 이라는 훨씬 싼 축을 이미 제시해 뒀고, **그 축을 고를지 판단하려면 분리 측정이 선행**이다. ⚠ 📄 92%·73%는 제조사 발표치이고 비교 기준은 raw에 **없음**.
  - 🧠 **덤으로 Stage2에 대한 방어 근거 하나**: 📄 (KR-W38-129) 아이온큐가 **탐색으로 풀던 조합 문제(양자회로)를 트랜스포머 생성으로 대체**한다(*4큐비트 34초 → 12큐비트 11분* 벽). ✅ Stage2(결정론 전개)가 정확히 그 *탐색/규칙 전개* 층인데, 🧠 여기를 생성으로 바꾸면 **속도 이득만큼 검증 비용이 같은 자리에 다시 생긴다** — 📄 (KR-W38-103) *인간이 읽을 수 없는 지능* · (KR-W38-78) *허용된 동사 목록* 과 같은 자리다. ✅ **결정론 층을 생성으로 바꾸지 않는 현재 설계를 지지**한다.
  - ✅ **서술 어휘 하나**: 📄 (KR-W38-122) *"**LLM의 등장으로 방대한 데이터를 원하는 형태로 가공(Processing)할 수 있게 됐다**"*([팜뉴스](https://www.pharmnews.com/news/articleView.html?idxno=306387)) — 🧠 이 제품이 하는 일(*지식소스 비정형 → 시나리오 정형*)의 **한 줄 가치 서술**로 [[프로젝트-포트폴리오]]에 쓸 수 있다. 📄 같은 날 (KR-W38-107) *EMR → 진료의뢰서 사전 학습 없이 99.25%* 가 그 주장의 수치 판본.
...
- 🆕 🔥 ⭐⭐ **(2026-09-16) 지식소스 설계의 「리스크 3축」이 한 주 안에 각각 다른 기사로 다 나왔다** ([[AI-주간-소식-2026-W38]] 6차 감사):
  - 📄 (KR-W38-01)이 *지식소스 + LLM 호출 + 생성* 구조를 **"RAG·에이전트 = 기업 AI 완성 공식"** 이라 정당화했는데, 같은 주에 **그 공식의 부작용 세 가지**가 각각 따로 보도됐다:
    - **① 데이터 품질** — 📄 (KR-W38-15) 파수AI: *"같은 문서가 여러 위치에 복사돼 있으면 **최신본 판단이 어렵다**."* → ✅ 단일 진실의 원천.
    - **② 구조 표현** — 📄 (KR-W38-41) 온톨로지 기반 경영 질의 · 🆕 📄 (KR-W38-65) 아이티센엔텍 공군 C4I *"AgentGO와 **온톨로지** 개념 개발 및 초거대 LLM 연구 성과를 융합"*([뉴스핌](https://n.news.naver.com/mnews/article/030/0003467102?sid=105)). 🔥 **이틀 사이 완전히 다른 두 산업(경영·지휘통제)에서 같은 조합이 나왔다** — 🧠 *지식소스에 **문서를 넣을까 관계 구조를 넣을까***가 선택 항목이라는 업계 근거 2건. ✅ **챗봇 시나리오는 본질이 「노드 간 관계」라 후자에 가깝다** — 지식소스를 문서 임베딩이 아니라 그래프로 두는 설계의 근거.
    - **③ 보안 경계** — 🆕 📄 (KR-W38-71) *"**RAG 시스템 도입** 등에 따른 **AI 방화벽** 필요성"*([뉴스핌](https://n.news.naver.com/mnews/article/030/0003467190?sid=101)). 🧠 사내 지식이 모델 경계를 넘나드는 데서 오는 리스크로, ✅ 이 프로젝트의 지식소스 3종 중 `문서 검색(docs)`·`혼합(mixed)`에만 걸린다(`일반 LLM(general)`은 해당 없음).
  - ✅ **적용안**: 지식소스 옵션 문서(설문 UI 안내·매뉴얼 A-2)에 **3축을 선택 기준으로 명시**한다 — *"문서 최신성을 누가 보증하나 / 관계가 중요한 도메인인가 / 이 문서가 모델 경계를 넘어도 되나"*. ⚠ 📄 *"AI 방화벽"* 이 확립된 제품 범주인지는 raw에 **없음**(기고 1편, 문제 제기 단계).
- 🆕 ⭐ **(2026-09-16) 비용 지렛대는 「호출 횟수」보다 「컨텍스트 길이」일 수 있다** (📄 (KR-W38-72) · 6차 감사):
  - 📄 조선일보 — *"**대규모 언어 모델이 답변 생성 시 활용하는 KV 캐시(Key-Value Cache)**"* 기술 설명([023](https://n.news.naver.com/mnews/article/023/0003998702?sid=105)). 📄 (KR-W38-04) 딥시크 V4.1이 *"긴 프롬프트나 **에이전트 반복 작업**에서 **캐시를 1/4로 압축**"* 해 서빙 비용을 줄인다고 한 그 캐시다.
  - 🧠 **추론**: KV 캐시가 컨텍스트 길이에 비례해 메모리를 먹는 구조라면, 📄 (KR-W38-34) *"멀티에이전트 = 채팅 대비 15배 토큰"* 의 비용은 **단순 호출수 × 단가가 아니라 각 호출의 컨텍스트 길이에 더 민감**하다. ✅ Stage별 반복 호출을 줄이는 것보다 **Stage마다 넘기는 컨텍스트를 깎는 쪽**이 지렛대가 클 수 있다((W36-EN3) *캐시를 깨지 않는 컨텍스트 관리*와 같은 결론).
  - ⚠ 🚨 **이건 판단이다** — 📄 캐시 재사용 조건·실제 과금 반영 방식은 raw에 **없다**. ✅ 실제 API 과금 구조로 검증한 뒤에만 비용 공식에 넣는다.
- 🆕 🔥 ⭐⭐⭐ **(2026-09-17) 위 ②「구조 표현」 축에 3호가 붙었는데, 이번엔 개념어가 아니라 *구현 스택*이다** ([[AI-주간-소식-2026-W38]] (KR-W38-82)+(KR-W38-100) · 8차 감사):
  - 📄 손오공-AGEDB 공동 R&D — *"**포스트그레SQL·아파치 에이지(Apache AGE) 기반 '관계 분석용 DB' 계층** 설계 검토. **AI 에이전트와 결합해 추론 효율 극대화**"*([딜사이트](https://dealsite.co.kr/articles/169199) / [이투데이](https://www.etoday.co.kr/news/view/2626582)). 📄 같은 회사가 B2B 중고차 플랫폼에서 **자체 에이전트 기반 자동화 ERP**를 운영 중이다.
  - 🔥 ✅ **앞 2호와 격이 다른 지점**: 📄 (KR-W38-41) 경영 질의·(KR-W38-65) 공군 C4I는 *"온톨로지"* 라는 **개념어**였다. 이건 **엔진 이름**이다 — 🧠 그래서 이 건만 바로 실행 가능한 정보다.
  - 🔥 ✅ **이 프로젝트에 직접 걸리는 이유**: 🧠 백엔드가 **Supabase(= PostgreSQL)** 이므로, *지식소스를 문서 임베딩에서 그래프로 옮기는 경로*가 **별도 그래프 DB 도입이 아니라 같은 엔진의 확장(Apache AGE)** 위에 있다. ✅ 위 ② 축이 *"챗봇 시나리오는 본질이 노드 간 관계"* 라고 적어 둔 자리에 **마이그레이션 비용이 낮은 선택지**가 하나 생겼다.
  - ⚠ 🚨 📄 **raw는 타사의 「검토」 단계 보도**다. 성능·도입 여부·AGE의 프로덕션 성숙도는 raw에 **없음** — 🧠 실측 없이 설계 결정으로 올리지 않는다.
- 🆕 🔥 ⭐⭐ **(2026-09-17) 시나리오 품질을 「성공률 N%」 하나로 보면 안 되는 이유가 남의 도메인에서 문장으로 나왔다** (📄 (KR-W38-90) · 8차 감사):
  - 📄 의료 AI 권고 — *"**전체 정확도**를 넘어 **성별·연령·인종 등 하위집단별 데이터 구성과 성능 차이** 확인은 물론, **생성형 AI가 만든 의료영상의 의학적·해부학적 정확성**까지 연구 전 과정에서 점검"*([닥터스타임즈](http://www.doctorstimes.com/news/articleView.html?idxno=240057)).
  - 🧠 **도메인은 의료지만 명제는 평가 설계다**: *단일 집계 지표는 하위집단의 성능 차이를 가린다.* ✅ 이 프로젝트 번역 — **시나리오 유형별로 쪼개서 잰다**(단순 FAQ ↔ 다분기 조건부 ↔ 예외·되묻기 분기). 🔥 전체 성공률이 올라가도 **다분기 쪽만 나빠지는 상태**가 지금 지표로는 보이지 않는다.
...
- 🆕 ✅ [[AI-주간-소식-2026-W39]] **7차 자동(2026-09-24)** — 📄 (KR-W39-92·93) **Opus 5.5 속도 30%↑·비용 40%↓ 독립 재확인** → §비용 축 처방 실행 1순위(모델 라우팅 레이어 분리) · 📄 (KR-W39-102) **Zeb 초경량 모델** → Stage1 판단 전용 저비용 선택지 후보 추가
