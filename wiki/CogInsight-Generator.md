...
> 🔶 **현재 개발 = v0.7.0 진행 중**(브랜치 `feat/v0.7.0-combined-dev`, main 대비 58커밋 · **프로덕션 무배포**). 자연어 수정(`nl-quickgen`·`nl-esd-extract`)·설문 기반 수정(`survey-edit`)·결과 ESD 편집(`esd-result-edit`) 축을 통합 dev 브랜치에 머지했고, 2026-08-25~26은 백로그에서 끌어올린 **다이어그램 편집**에 집중 — 연결선 조건 편집(가이드 빌더)·고립 노드 검증·노드 내용(기능) 편집(타입별 편집기)·반복 본문 편집·조건 없는 분기 확정 차단. 2026-08-27 회귀 수정 3건(조건 편집기 시드 `9cff45d` · 되돌리기가 확정 이력을 롤백 `adbb01a` · 조건 없는 갈래 '그 외' 표시+else 순서 자동 정렬 `ab610cb`). 이어서 **시나리오 헤더 이름·진입 조건 표시**(스펙+계획+SDD 8태스크, `37d8716`~`a31bde8` 12커밋): LLM이 FlowSpec 진입 노드에 `scenarioName`을 태깅해 `generation_tiers.scenarios`로 흐르고(**`generated_json`은 무변경 — 솔루션 임포터가 모르는 필드를 거부**), 프론트는 번들→조건파생→루트라벨 3단 폴백이라 기존 결과도 즉시 개선. **런타임이 `then` 순서대로 조건을 확인함이 확정**되어 else 갈래 정렬을 생성기·편집기 양쪽에 걸었다. 구조 다이어그램과 사용자 흐름도가 같은 이름을 쓰도록 통일. dev 슬러그 배포+실측 완료(프로덕션 미배포). ⚠ 블록 E(PR #115)는 아직 미머지 · dev 리셋 미수행. 그 이전 릴리스: v0.5.0(입력 소스 다양화, PR #111)·v0.4.2(결과 확인 창 UI, PR #108)·v0.4.1(결과 탭 사용자별, PR #107)·v0.4.0(플로우 시각화, PR #106).
