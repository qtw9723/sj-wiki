...
- **재수정(커밋 `e209d26`)**: 정중앙 배치로도 **좁은 간격(반복 본문 ~22px)에선 텍스트가 여전히 겹침** → 턴 경계는 **텍스트 라벨 없이 점선(파란색)으로만** 표시(의미는 범례가 설명). 분기 조건 라벨은 넓은 구간이라 유지. 🧠 dev는 소스 파일 직접 편집 → vite HMR로 반영(브라우저 강력 새로고침 필요할 수 있음).
