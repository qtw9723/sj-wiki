...
## 진행 현황
> 📄 **현재 프로덕션 = v0.7.1**(설문 기반 수정 버그 + 생성 소요 시간 표시, 2026-08-27 — PR #117 main `7c9545c`, tag v0.7.1). 설문 기반 수정의 시나리오 입력이 `[object Object]`로 뭉개지고 **시나리오 검증이 통째로 죽어 있던** 버그 수정(템플릿 결합을 순수 헬퍼 `attachTemplate`으로 추출해 생성 설문과 공유), LLM 사용량에 생성 소요 시간 표시(측정 이전 결과는 줄 숨김). DB·마이그레이션 무변경, 프로덕션 함수 1종 배포. 직전 v0.7.0(생성 결과 수정 + 다이어그램 편집 + 시나리오 표기·분기 안전장치, PR #116 `b43d897`).
