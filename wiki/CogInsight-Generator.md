...
## 진행 현황
> 📄 **현재 프로덕션 = v0.8.0**(NLU 학습 데이터(인텐트·엔티티) 생성 + CSV 다운로드, 2026-08-28 — PR #118 main `668e3a7`, tag v0.8.0). 봇 생성 시 시나리오·ESD에서 NLU 학습 데이터를 함께 파생해 결과 상세에서 솔루션 업로드용 `intents.csv`·`entities.csv`를 내려받는다. 인텐트 = 시나리오 1:1(대분류) · 엔티티 = 세부 구분. `generated_json` 스키마 무변경(산출물은 `generation_tiers.nlp` 번들에만). 조건을 인텐트·엔티티에 바인딩하는 작업은 v0.8.1. 직전 v0.7.1(설문 기반 수정 버그 + 생성 소요 시간 표시, PR #117 `7c9545c`).
