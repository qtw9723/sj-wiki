...
> 📄 **현재 프로덕션 = v0.10.0**(생성 데이터 관리 & 자산 모듈화, 2026-09-11 — PR #126 머지 main `4ee0367`, tag v0.10.0 + GitHub Release). 항목 6개 전부 완료: 번들 계약·중복 키 정리·NLP 만들기 도구(`/nlp`)·모듈 이동·**자산 라이브러리**·결과 보존 정책. 핵심은 **부산물이던 NLP·ESD·API를 독립 자산으로** 올린 것 — 보관한 결과에서 시나리오 하나를 자산째로 등록하고, 여러 개를 골라 조립해 새 결과로 저장한다(조립물도 기존 다운로드 그대로). 신규 테이블 `coginsight_scenario_assets`(+dev 미러·RLS), 신규 함수 `scenario-assets`(+`-dev`). 용어 **NLU→NLP 전면 리네임**(PR #125). 마이그레이션 1건 적용 · 영향 함수 12종 배포 · Vercel·사내 온프렘·공개 개요 페이지·아티팩트 모두 반영 완료.
> 🔶 **현재 개발 = 없음.** 다음은 v0.10.1(부가 도구: 데이터 비교) 또는 v0.11.0(로직 정합화·부채 정리) 착수 예정. ⚠ v0.11.0 후보로 이월된 것: 자산 삭제 UI·관리자 삭제 경로(서버측 관리자 토큰 검증이 저장소에 없다), 동명 시나리오 인텐트 겹침, `useResults` 두 벌 통합, few-shot 주입(B단계).
