...
> ✅ (2026-07-15) **v0.4.0 — 플로우 시각화 프로덕션 릴리스** (PR #106 머지 main `52a0f59`, tag v0.4.0 + GitHub Release). **현재 프로덕션 = v0.4.0**. react-flow 캔버스 뷰어(구조 다이어그램 + 사용자 흐름도) + PNG·풀스크린, **진입점 4곳**(레퍼런스 2 + 생성 결과 2). 프론트 전용(Vercel 자동 배포, DB·엣지함수 무변경). 검증 green(deno 260/0, build 2065). 공유 개요 페이지·링크 노트 갱신. 상세는 [[#진행사항 업데이트 로그|진행 로그 2026-07-15 릴리스]].
...
## 진행 현황
> 📄 **현재 프로덕션 = v0.4.0**(플로우 시각화, 2026-07-15 릴리스 — PR #106 머지 main `52a0f59`, tag v0.4.0). 프론트 전용이라 Vercel 자동 배포(DB·엣지함수 무변경). 공유 개요 페이지·링크 노트 갱신 완료. 아래 진행 로그 2026-07-15 릴리스 참고.
...
- **공유 노트 최신화(사용자 요구)**: ⓐ **공개 개요 페이지**(https://coginsight-overview.vercel.app) — v0.4.0 "예정→릴리스" 전환(인트로·버전표·푸터 노트), v0.4.0 섹션·통계(795커밋) 추가 후 `vercel --prod` 재배포·200·라이브 반영 검증. ⓑ **[[CogInsight-Generator-링크]]** 위키 노트 v0.4.0 갱신(최신 릴리스 링크·개요 버전). ⚠ 개요 아티팩트(1e30660a)는 배포본 직접 수정이라 v0.3.0 기준으로 남음(드리프트 — 링크 노트에 기록).
