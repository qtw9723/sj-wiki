...
> [!note] 다음 방향 (2026-07-20 브레인스토밍 확정 — SoT는 저장소 `ROADMAP.md` "다음 방향" 절)
> 🧠 큰 전환: **"정해진 순서 플로우 → LLM 주도 상황 적응형 생성"**.
> 1. **LLM 노드 상황 적응형 진행** — 하나의 LLM 노드가 사용자 발화에 따라 다양한 일(수집·도구 호출·라우팅·응대)을 상황에 맞게 실행하고 조건 충족 시 탈출. 현재의 다중노드 `llmFlowChain`을 **단일 agentic 노드**로 일반화. (열린 질문: "LLM이 설계, 코드가 구조 강제" 원칙과의 관계.)
> 2. **컨디션 노드를 진입 라우팅 전용으로** — 전용 컨디션 노드는 시나리오 진입(첫 발화 라우팅)에만, 중간 분기는 개별(내용) 노드 자체의 조건으로.
> 3. **라이브러리 기반 생성 UX** — 다중 선택 → 관련 추가 설정 수집 → 생성 전 예상 결과 미리보기.
> 버전·우선순위 미확정(기존 v0.5 후보 '다이어그램 편집'과 병존). 항목 1은 백로그의 "수집형 체인 분기·이탈 tool·`_llm.terminate`"를 통합.
