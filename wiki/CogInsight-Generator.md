...
## 진행 현황
> 📄 **현재 프로덕션 = v0.7.0**(생성 결과 수정 + 다이어그램 편집 + 시나리오 표기·분기 안전장치, 2026-08-27 — PR #116 main `b43d897`, tag v0.7.0). 결과를 그 자리에서 고치는 3방식(자연어·설문 재편집·대화형) + 결과 ESD·API 편집, 구조 다이어그램 직접 편집(노드 추가·삭제·연결·내용·연결선 조건·반복 본문 + 재검증 5종), 시나리오 헤더에 이름 표시(구조도·흐름도 통일)·진입 조건 표시, 조건 없는 갈래 `그 외` 표기 + **런타임이 `then` 순서대로 조건을 확인**하므로 else 갈래 결정론 정렬(생성기·편집기 양쪽), 문서→ESD QA 12건. `generated_json` 스키마 무변경(임포터가 미지 필드 거부 → 이름은 `generation_tiers.scenarios` 번들에만). DB·마이그레이션 무변경, 프로덕션 함수 5종 배포. 직전 v0.6.0(ESD·API 선행 정의·매핑, PR #114 `1e5d53d`).
