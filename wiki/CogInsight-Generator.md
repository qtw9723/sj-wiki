...
## 진행 현황
> 📄 **현재 프로덕션 = v0.5.0**(입력 소스 다양화, 2026-07-28 — PR #111 main `1e3237d`, tag v0.5.0). 설문 외 3개 입구 추가(엑셀→ESD, API문서→api_def, 자연어→시나리오, POC문서→생성) + 신규 엣지함수 2종(`poc-doc-parse`·`nl-scenario-parse`) 배포, Vercel 라이브 확인. 사내 온프렘 배포·공개 개요 페이지 반영 완료, 릴리스 규약 5단계화(⑤ 공개 개요 필수). DB 무변경. ⚠ dev 리셋만 다음 버전 착수 시로 이월. 직전 v0.4.2(결과 확인 창 UI, PR #108 `2986e1f`)·v0.4.1(결과 탭 사용자별, PR #107)·v0.4.0(플로우 시각화, PR #106). 아래 진행 로그 참고.
...
> [!note] 다음 버전 계획 (2026-07-20 브레인스토밍 → 2026-07-21 v0.5.0·v0.6.0 확정 → 2026-07-29 v0.6.0/v0.7.0 재편 — SoT는 저장소 `ROADMAP.md`)
...
> 📄 **v0.6.0 (계획) = ESD 선행 생성 & 매핑 기반 설정** (2026-07-29 확정) — 생성 파이프라인을 "ESD(데이터/서비스 정의) 선행 → 그 ESD 기반 매핑 설정 → 생성"으로 재편. ESD를 ①직접 입력 ②문서 업로드 자동생성 두 방식으로 만들고(v0.5.0 `/convert`·`api_def` 재사용·통합), 확정 ESD를 기준으로 수집 변수·값·연동을 매핑. 저장 위치(새 테이블 vs 스키마 JSON 확장)·매핑 UX는 착수 시 결정.
> 📄 **v0.7.0 (계획) = 생성 결과 수정** (2026-07-29 확정, 기존 v0.6.0 자연어 수정 흡수·확장) — 생성된 flow/`generated_json`을 세 방식으로 수정: ①자연어 지시 ②설문 기반(원설문 항목 편집→반영) ③다이얼로그 기반(대화형 부분 편집). 전체 재생성 vs 부분 패치·불변식(`checkBot`) 재검증·수정 이력(diff)은 착수 시 결정. v0.4.0 다이어그램 편집(백로그)과 상보적.
...
> 📄 **운영 방침(2026-07-29)**: 확정 버전은 v0.6.0/v0.7.0 둘만 유지. 단 개발 중 **볼륨 여유가 있으면 백로그 항목을 하나씩 해당 버전 스코프에 포함**(상보적인 것 우선 — 예: 다이얼로그 수정 ↔ 다이어그램 편집).
