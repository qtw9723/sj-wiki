...
source: raw/projects/coginsight-generator.md, raw/ai-digest/2026-08-12.md, raw/ai-digest/2026-08-13.md, raw/ai-digest/naver-2026-08-20.md, raw/ai-digest/naver-2026-08-21.md, raw/ai-digest/2026-08-21.md, raw/ai-digest/2026-08-22.md, raw/ai-digest/naver-2026-08-28.md, raw/ai-digest/2026-09-01.md, raw/ai-digest/naver-2026-09-06.md (액션 슬롯 되돌림·영향 분석·중단 지표)
created: 2026-06-09
updated: 2026-09-06
---
...
> 🔶 **현재 개발 = v0.10.0**(생성 데이터 관리 & 자산 모듈화 — 항목1·2·4는 08-31~09-01, **항목3 "NLU 만들기 도구"는 2026-09-02 dev 검증까지 완료**, 항목5(자산 라이브러리)·6(결과 보존 정책) 미착수, **main 미머지·프로덕션 미배포**). 항목3 브랜치 `chore/v0.10.0-cycle-start`, 커밋 `19a29be`~`1c7fde8`. 상세는 [[#진행사항 업데이트 로그|진행 로그 2026-09-02]].
> ⚠ **정정(2026-09-02)**: 바로 위 줄이 "현재 개발 = 없음(v0.7.0 릴리스 완료). 다음 = v0.8.0 착수 예정"인 채로 낡아 있었다 — 실제로는 이미 v0.7.1·v0.8.0·v0.8.1·v0.9.0까지 릴리스되고 v0.10.0이 개발 중인데 갱신이 누락돼 있었다(이 세션에서 발견·정정).
...
## 진행사항 업데이트 로그
### 2026-09-02 — v0.10.0 항목3 "NLU 만들기 도구" dev 완료 (📄 dev 배포·curl·브라우저 실측, 프로덕션 미배포)
- **무엇을 만들었나**: 화면 `/nlu` — 봇을 만들지 않고 **자연어 업무 설명만으로** NLU 학습 데이터(인텐트·엔티티)를 만들어 솔루션 업로드용 CSV 2종(`intents.csv`·`entities.csv`)을 내려받는 단독 도구. 4단계(업무 설명 → 인텐트 확정 → 엔티티 확정 → 다운로드). 신규 엣지함수 `nlu-build`(+ `nlu-build-dev` 래퍼, `-dev` 래퍼가 13→14개). **DB 변경 없음.** 브랜치 `chore/v0.10.0-cycle-start`, 커밋 `19a29be`~`1c7fde8`(**main 미머지·미푸시**, 프로덕션 배포는 하지 않음 — 사용자 승인 사항).
- **구조**: **2단계 LLM 호출**(1단계 인텐트, 2단계 엔티티). **핵심 불변식**: 엔티티 단계 프롬프트에 확정 인텐트 전체(이름+예문)를 넘긴다 — 인텐트를 모르면 발화로 갈리는 축을 찾을 수 없고, 그걸 놓쳐 **분기 조건이 통째로 빈 결함이 v0.8.1에서 실제로 났다**(위 [[#로드맵 (현재) — 2026-08-28 재편|로드맵 "모듈화 시 주의"]] 참고).
- **NLU 작성 지침 공유**: 어터런스·동의어·엔티티 종류 지침이 `deriveNlpData.ts`의 `buildNlpPrompt`에 하드코딩돼 있었다. `_shared/nlpPromptParts.ts`로 뽑되 **기본값 있는 함수**로 만들어 경로별 어휘를 인자로 받는다(기본값=시나리오 경로 원문). 시나리오 경로 프롬프트 출력은 **바이트 동일**이며 스냅샷 테스트로 고정했다.
- **최종 리뷰가 잡은 Important 4건(전부 수정)** — 이 페이지가 실패·정정을 남기는 취지에 맞는 부분:
  1. 공유 상수에 **시나리오 전용 어휘**("시나리오의 수집 항목 선택지"·"시나리오의 기능 단어")가 남아 도구 프롬프트에도 실렸다 — 도구 경로엔 시나리오가 없는데 존재하지 않는 것에서 뽑으라고 지시하는 꼴. → 경로별 인자로 분리.
  2. 2단계에서 **인텐트 이름을 지우면** 검증을 통과한 뒤 서버 필터에서 조용히 버려졌다. 인텐트가 1개뿐이었다면 확정 인텐트 섹션이 통째로 빈 채 LLM에 가서 **핵심 불변식이 경고 없이 깨지는데 화면엔 정상으로 보였다**(v0.8.1 실패 모드 재현 경로). → 400 차단 + 버튼 잠금, 정규화 후 인텐트 0개(`"///"` 같은 이름)도 400.
  3. 화면에서 편집한 인텐트명이 **정규화를 우회**해 CSV로 직행했다 — `반품 접수`(공백)·`반품/교환`(슬래시)이 그대로 실려 시나리오 경로 산출물과 이름 규칙이 갈렸다. → `entities` 응답에 **정규화된 인텐트를 동봉**하고 화면이 채택, 이름 규칙은 `_shared/intentName.ts` 단일 소스 유지.
  4. `BTN_PRIMARY` **5번째 복제**를 새로 만들었는데 hover 색이 기존 5곳(`#003d99`)과 달리 `#0047B3`로 **이미 값이 갈려 있었다**. → 저장소 프리미티브 `ui/Button`·`ui/StepBar`·`ui/PageHeader`로 전환(기존 5곳 정리는 별건으로 남음 — v0.12.0 "UI 전면 정리" 후보).
- **dev 실측(재배포 후 재검증, 2026-09-02)**: `nlu-build-dev` 배포. curl 5종 — 잘못된 step 400 / 빈 인텐트명 400 / 정규화 후 0개 400 / 1단계 인텐트 3개(반품접수·배송조회·매장안내, 각 예문 24개·공백/특수문자 없음) / 2단계에서 `반품 접수`(공백)를 보내면 응답에 `entities`+`intents`가 함께 오고 이름이 `반품접수`로 정규화됨. 브라우저 `/nlu` 4단계 7항목 확인(버전 배지 DEV·StepBar 단계 표시·빈 이름 시 버튼 잠금·정규화가 화면에 보임·3→2→재생성 왕복·CSV 2종 다운로드·`undefined`/`NaN`/`[object Object]` 없음·콘솔 에러 없음). CSV 규격: BOM 있음, CRLF 없음, `entities.csv` 13컬럼, 인텐트명에 공백/슬래시 없음. 검증: `_shared` 48·`nlu-build` 24·`src/lib` 850·생성기 286 전부 0 failed, `deno check` 클린, `npm run build` 통과.
- **남은 것/주의**: v0.10.0의 나머지 항목 **5(자산 라이브러리)**·**6(결과 보존 정책)** 미착수(항목 1·2·4는 08-31~09-01 완료). `nlu-build/index.ts`가 최상단에서 `serve()`를 불러 **핸들러를 떼어낼 seam이 없어** 응답 계약 변경에 단위 RED/GREEN을 만들지 못하고 대조 실행 출력으로 대체(seam이 생기면 후속 과제). 저장소 전체 `deno test supabase/functions/`는 `test/rules.test.ts` 8건 등 **선재 실패**로 붉다(이 브랜치와 무관, main 조상에서 유래).
...
## 의외의 연결점 (🧠 판단 영역)
- 🆕 🔥 🚨 ⭐⭐⭐ **(2026-09-06) 액션 슬롯에 「되돌림」 칸이 필요해졌다 — 에이전트가 국내에서 처음으로 실제 결제를 했다** (📄 [[AI-주간-소식-2026-W36]] 13차 감사):
  - 📄 **(KR-W36-95) 신한카드 — 국내 카드사 최초로 마스터카드와 'AI 에이전트 페이' 실거래 성공**, *"AI가 사용자 목적에 맞게 자동으로 결제를 처리"*([apnews](https://www.apnews.kr/news/articleView.html?idxno=3051028)). ⚠ 거래 규모·한도·인증 방식·책임 주체는 raw에 **없음(불명)**.
  - 🚨 ✅ 🧠 **바로 아래 (2026-09-05) 항목이 액션 슬롯을 *"외부 API 호출 정의 + 실패 폴백 + 권한"* 3칸으로 잡았는데, 하루 만에 4번째 칸이 필요해졌다**: **되돌림(compensation)** — *취소·재시도·멱등키*. 🧠 실패 폴백은 *"실행이 안 됐을 때"* 를 다루고, 되돌림은 **"실행이 됐는데 그러면 안 됐을 때"** 를 다룬다. 📄 부작용이 로그가 아니라 **돈**이면 둘은 다른 칸이다.
  - 🧠 **(KR-W36-70) 보상 해킹 처방이 이 시점부터 법적 책임 영역이다**: 결과만 채점하는 게이트 + 되돌릴 수 없는 외부 동작 = 우회의 비용이 **실제 손해**다. ✅ 즉 위 (2026-09-04) 항목의 *"중간 과정 검사 1칸"* 은 **품질 옵션이 아니라 실행 기능의 전제 조건**으로 승격된다.
- 🆕 🔥 🚨 ⭐⭐⭐ **(2026-09-06) 에이전트 품질 기준이 「무엇을 생성하는가」에서 「변경이 어디에 영향을 미치는가」로 이동했다** (📄 (KR-W36-94) 에이아이웍스):
  - 📄 *"코드 분석·SQL 분석 **전문 하위 에이전트 + 마스터 에이전트** 조합의 멀티에이전트 구조로 **코드 변경 영향 분석** 솔루션 개발. **샌드박스 실행 및 근거 제시** 기능 포함"*([it.chosun](https://it.chosun.com/news/articleView.html?idxno=2023092169376)). ⚠ 정확도·도입 사례·가격은 raw에 **없음**.
  - ✅ 🧠 **이 프로젝트에 그대로 걸린다**: 시나리오를 고칠 때 *"고친 결과"* 만 보여 주는 것과 **"이 분기를 바꾸면 어떤 다른 분기·규칙(`solution_rules`)이 함께 흔들리는가"** 를 먼저 계산해 주는 것은 다른 제품이다. 🧠 규칙이 DB-구동(SoT)이라 **영향 범위를 실제로 질의할 수 있다** — 이 프로젝트는 이미 그 계산을 할 재료를 갖고 있다.
  - 🔥 🧠 **(KR-W36-68) SafeQL *"틀린 곳만 부분 교정"* 과 한 쌍이다** — 둘 다 **전체를 다시 만들지 않는다**는 같은 절약 원리이고, 부분 교정은 *사후*, 영향 분석은 *사전*이다. ✅ 아래 (2026-09-04) 부분 교정 원칙과 **같은 칸에서 관리**한다.
  - ✅ 📄 *"**근거 제시**"* 가 상용 스펙 문장에 들어왔다 = [[에이전트-자동화-도구]]의 **증거 층**이 제품 기능으로 팔리기 시작했다는 뜻. 🧠 이 프로젝트의 *"왜 이 분기인가"* 기록은 유행이 아니라 **경쟁 기능**이다.
- 🆕 ⭐⭐ **(2026-09-06) 멀티에이전트 성능 지표는 속도가 아니라 「중단 횟수」다** (📄 (KR-W36-104) GPT-6 아스트라):
  - 📄 *"메인 에이전트가 하위 모델 작업을 **감독**하는 구조로 프롬프트 토큰 소모 **30% 절감**. 그러나 **멀티스텝 수행 중 환각 문제로 작업 흐름이 끊기는** 사례도 보고"*([aitimes](https://www.aitimes.com/news/articleView.html?idxno=214891)).
  - 🔥 ✅ **(W36-EN3) 비용 공식 `호출수 × 토큰 × (1−캐시적중률)` 의 세 항 중 둘에 이제 실측 처방이 붙었다**: 📄 (KR-W36-68) SafeQL = **호출수**, 📄 오늘 감독 구조 = **토큰(−30%)**. 🧠 6단계 파이프라인은 이미 *마스터 → 단계별* 구조라 이 절감이 **원리적으로 적용되는 형태**다.
  - 🔥 ✅ **그리고 이건 (KR-W36-76) *"환각 = 기억 인출 실패"* 의 현장 증거다** — 🧠 멀티스텝에서 흐름이 끊기는 지점 = **앞 단계 결과를 인출해야 하는 지점**. 아래 (2026-09-04) 항목이 이론으로 말한 것이 **한 회차 만에 사례로 확인**됐다.
  - ✅ 🧠 **측정 규칙**: 멀티에이전트 손익은 **`토큰 절감 − 중단 복구 비용`** 인데 📄 후자는 아무도 수치를 안 냈다 → 이 프로젝트에서 파이프라인을 손볼 때 재는 것은 *생성 시간*이 아니라 **단계 중단·재시도 횟수**여야 한다.
- 🆕 🔥 🚨 ⭐⭐⭐ **(2026-09-05) 다음 단계가 외부에서 정의됐다 — 「답을 고르는 툴」에서 「답 뒤의 실행까지 잇는 툴」로** (📄 [[AI-주간-소식-2026-W36]] 11차 감사):
  - 📄 **하루에 세 층이 같은 말을 했다**: (KR-W36-81) 이통 3사 *"질문 답변을 넘어 실제 업무 처리까지 연결하는 **행동형 AI 에이전트**"*(SKT 에이닷엑스 K2, 매개변수 6880억) · (KR-W36-91) 애플 *"메시지·사진·메일을 이해하고 **여러 앱을 오가며** 업무 대신 처리, 연내 시범"* · (KR-W36-93) 삼성SDS *"GPU·NPU부터 AI 에이전트·플랫폼까지 **AI 풀스택**"*. 🧠 통신·OS·SI 세 층이 같은 날 수렴했다 = **개별 전략이 아니라 산업 방향**.
  - ✅ 🧠 **이 프로젝트에 걸리는 지점**: 지금 산출물(Dialog JSON)은 *"어떤 발화에 어떤 답을 하는가"* 까지다. 행동형은 **"답을 한 뒤 어떤 외부 동작을 호출하는가"** 를 요구한다 → **액션 슬롯**(외부 API 호출 정의 + 실패 폴백 + 권한)이 다음 설계 결정이다.
  - 🚨 ⚠ **단, 이 프로젝트 고유의 제약이 있고 외부 사례엔 답이 없다**: 대상 챗봇은 **사용자 발화 ↔ 봇 응답 엄격 교대**라 봇이 한 번에 두 번 말할 수 없다. 🧠 그래서 액션 실행 중 상태를 *"봇의 두 번째 발화"* 로 만들면 모델이 깨진다 → **실행 결과를 다음 사용자 턴 뒤에 붙이거나, 실행 중 상태를 턴이 아닌 메타 필드로** 둬야 한다.
  - ✅ **그리고 실행이 붙는 순간 위 (KR-W36-70) 보상 해킹 처방이 필수가 된다** 🧠: 결과만 채점하는 게이트에 **되돌릴 수 없는 외부 동작**(발송·결제·접수)이 연결되면 우회의 비용이 로그가 아니라 **실제 부작용**으로 나온다 → 액션 슬롯에는 **중간 과정 검사 1칸**이 옵션이 아니라 전제다.
- 🆕 ⭐⭐ **(2026-09-05) 「사용자가 왜 이 분기인지 볼 수 있는가」가 조달 요건 쪽으로 내려오고 있다** (📄 (KR-W36-84) 포스텍 국책사업 2건):
  - 📄 *"멀티모달 AI와 다수 AI 에이전트가 협력하는 **인간참여형(Human-in-the-Loop) Agentic AI**, **설명가능성·안전성**을 갖춘 신뢰 가능한 플랫폼."* ⚠ 의료 도메인이며 평가 기준·과제 상세는 raw에 **없음**.
  - 🧠 패턴 자체는 도메인 무관이다: **다수 에이전트 협력 → 인간이 중간 검토 → 설명 가능한 출력**. ✅ 이 프로젝트의 최소 형태는 **6단계 파이프라인의 각 단계가 "이 분기를 왜 만들었는가"를 남기는 것** — 🧠 이게 (W36-EN2) 관제실·(W36-EN6) 증거 층의 프로젝트 내 판본이고, 위 보상 해킹 처방과 **같은 한 칸**이다(따로 만들 필요 없음).
- 🆕 🔥 🚨 ⭐⭐⭐ **(2026-09-04) 환각 대책의 방향이 뒤집혔다 — 「지식을 더 넣기」가 아니라 「인출되게 배치하기」** (📄 [[AI-주간-소식-2026-W36]] 9차 감사 (KR-W36-76)):
  - 📄 구글: *"**LLM 환각의 원인은 지식 부족이 아니라 「기억 인출」 실패**"* (`raw/ai-digest/naver-2026-09-04.md`, AI타임스 `[9월3일]` 다이제스트 내부 · ⚠ 논문 형태·실험 근거·원어는 raw에 **없음**).
  - ✅ 🧠 **적용 — 이 프로젝트가 손댈 수 있는 층에 정확히 걸린다**: 환각이 *모델이 몰라서* 라면 처방은 모델 교체·지식 주입이고 이 파이프라인 밖의 일이다. 📄 그런데 원인이 **인출 실패**면 처방은 **6단계 파이프라인이 각 단계에 무엇을 어떤 순서로 넣는가**로 내려온다 — 🧠 즉 **프롬프트 구성이 환각률을 직접 좌우한다**.
  - 🔥 ✅ **(W36-EN3) TokenPilot과 합치면 「컨텍스트 중간 삭제 금지」가 두 이유로 확정된다**: ① 프리픽스가 바뀌어 **KV 캐시가 죽는다**(비용 ≈ 호출수 × 토큰 × (1−캐시적중률)) ② **인출 단서가 사라진다**(정확도). ✅ **규칙**: 긴 시나리오 세션에서 압축이 필요하면 **중간을 잘라내지 말고, 요약을 「새 접두 상태」로 한 번 고정한 뒤 그 뒤로만 덧붙인다**(mutation 아니라 checkpoint).
  - 🧠 **우선순위 함의**: 📄 W36 한 주에 모델이 **4개** 출시됐다(Fable 5.1 · 메타 뮤즈 스파크 1.3 · Gemini 3.8 · GPT-6 아스트라). 🧠 모델 핀을 따라가는 유지비는 계속 오르는데 **인출 설계는 한 번 잘 하면 모델을 넘어 남는다** → 같은 시간을 후자에 쓰는 편이 낫다([[내-MCP-커넥터-환경]] §5-2와 같은 결론).
- 🆕 🔥 🚨 ⭐⭐⭐ **(2026-09-04) 오류 처리 원칙 — 전체 재생성보다 「부분 교정」을 먼저 시도한다** (📄 (KR-W36-68) KAIST SafeQL):
  - 📄 *"기존에는 SQL 오류 발생 시 전체를 LLM에 재전송해야 했으나 SafeQL은 **틀린 부분만 교정** — LLM 반복 호출 비용·시간 절감, 데이터 검색 오류 **87% 해결**"* (기사 3건 = 1사건). ⚠ 구현 방법·지원 DB·비교 기준 모델은 raw에 **없음**.
  - ✅ 🧠 **설계 원칙으로 승격**: 6단계 파이프라인의 어느 단계가 실패해도 **그 단계만 재실행**할 수 있어야 한다 — 지금처럼 단계 실패가 전체 재생성을 부르면 **비용 공식의 「호출수」 항을 그대로 곱한다**. 🧠 이건 위 (KR-W36-76)/EN3의 checkpoint 처방과 **같은 구조**다: 상태를 통째로 다시 만들지 말고 **델타만 고친다**.
  - ✅ 파생: [[mailer|CS SmartHub]]에서도 외부 DB 쿼리 실패 시 시나리오 전체 재시작이 아니라 쿼리 부분만 재생성.
- 🆕 🔥 ⭐⭐⭐ **(2026-09-04) 보상 해킹(Reward Hacking) — 「결과만 평가」하는 게이트는 우회된다** (📄 (KR-W36-70) 앤트로픽):
  - 📄 앤트로픽: *"**결과만 평가하면 AI가 통제를 우회**한다"* — 보상 해킹 위험 경고. 📄 NYT는 앤트로픽을 *"AI 에이전트 감독 메커니즘을 강화한 몇 안 되는 기업"* 으로 언급.
  - 🚨 ✅ 🧠 **이 프로젝트의 QA 게이트 설계에 직접 걸린다**: 위 ㉕(*"판정을 결정론 게이트로"*)·㊲(*"홀드아웃 채점"*)은 **둘 다 결과 기반 판정**이다 — 🧠 채점 점수만 보상 신호로 쓰면 **점수를 잘 받는 시나리오**(루브릭의 빈틈을 찾은 산출물)가 선택될 수 있다. ✅ 처방: 홀드아웃 채점에 **중간 과정 검사 1칸**을 붙인다(어떤 근거로 그 분기를 만들었나) — 🧠 이것이 (W36-EN2) 관제실·(W36-EN6) 증거 층이 이 프로젝트 안에서 갖는 최소 형태다.
  - 🧠 **모델 선택의 안전 논거**: 📄 (KR-W36-69) GPT-6 아스트라가 **자율화**를 밀어붙이는 동안 앤트로픽은 **감독**을 강조한다 → 판정·감사가 중요한 워크로드에서 Fable 계열을 고르는 근거로 인용 가능(⚠ 벤치마크 비교는 raw에 **없음** — 성능 우열 단정 금지).
...
- ⭐⭐ 🚨 **🆕 (2026-08-28) 판정을 LLM에 남길 수밖에 없을 때의 형태가 실측으로 좁혀졌다 — self-check는 안 되고, 독립 패스는 된다** (📄 [[AI-주간-소식-2026-W35]] 13차 재감사):
  - 📄 **이 vault가 낸 실측 대비**: 자동 위키화 패스의 **자체 점검 12회 = 12회 모두 무결 보고**, 사용자 요청 **재감사 5회(3·5·7·11·13차) = 5회 모두 오류 발견**. 📄 특히 12차는 자체 점검 **12항목 전부에 정수를 적고 검산 4방향을 통과한 상태에서 4항목이 거짓**이었다(흡수 근거 칸에 *"다른 소재이나 … 병기"* 라고 **적어 놓고** 통과).
  - 📄 같은 날 뉴스가 그 구조를 팔고 있었다: **(KR-W35-92) 식약처 AI심사관**(GPT5-mini·Claude Sonnet 4.5·Qwen3-VL을 웃도는 성능으로 **규제 문서를 심사**) · **(KR-W35-106) KB금융**(인프라 **사업성·리스크 심사**) · **(KR-W35-100) 딥노이드**(뇌MRI **정량 판독**) = 🧠 **「AI가 심사·판정 업무로 올라간다」 클러스터**. 공통점은 심사관이 **심사 대상과 다른 주체**라는 것.
  - ✅ 🧠 **적용 규칙(연결점 ㉕의 보강)**: 어떤 단계의 LLM이 **판정자로 남을 수밖에 없으면**, 검증을 *같은 실행 안의 체크리스트*가 아니라 **별개 실행**으로 옮긴다 — 생성 프롬프트와 검증 프롬프트를 분리하고, 검증 측에 생성 측 근거를 **「주장」으로 넣어 반박시킨다**. 📄 체크리스트를 「불리언 → 정수 카운터」로 조인 처방(5·11차)은 **값을 만드는 측정 행위를 검증하지 못한다** → 카운터 옆에 **그 값을 낸 명령·출력**을 요구해야 한다.
  - ⚠ 🧠 **위 ㉚(8/7)과 충돌하지 않게 구분할 것**: ㉚이 금지한 건 *"여러 모델에게 물어 **다수결·합의**"*(상호작용이 편향을 증폭)이고, 여기서 말하는 건 **단일 독립 심사 패스**다. **독립성은 「모델 수」가 아니라 「실행 경계」에서 온다** — 같은 모델이라도 실행이 분리되고 입력이 「검증할 주장」으로 재구성되면 유효하고, 모델을 늘려 투표시키면 무효다.
  - ✅ **적용 대상**: 시나리오 QA 채점(㊲ 홀드아웃 루프)의 **채점자**, `solution_rules` 자동 제안의 승인, 그리고 [[mailer|CS SmartHub]]의 티켓 자동 분류·자동 승인 지점.
- ⭐ **🆕 (2026-08-28) 포지셔닝 — 「진입점이 되기」와 「진입점 안으로 들어가기」가 둘 다 시장에서 인정됐다** (📄 [[AI-주간-소식-2026-W35]]):
  - 📄 3주 서사: **세일즈포스가 태블로 MCP로 Claude Code를 자기 제품에 붙였다**([[AI-주간-소식-2026-W34]] (KR84), 8/21) → **세일즈포스가 Claude 안으로 들어갔다**((KR-W35-91) *"세일즈포스가 '클로드' 안으로 들어왔다"*, 37개 사전 탑재 영업 스킬, 8/27) → 📄 주가 **+22.58%**(8/28).
  - 📄 한국 사례: **(KR-W35-93) 쿠콘**이 금융·공공 데이터를 **MCP 형태로 공급**해 AI 에이전트가 직접 가져가게 했다.
  - 🧠 **적용**: 이 프로젝트의 경로가 「자체 UI가 진입점」 하나만이 아니다 — **Claude/MCP 생태계 안의 「챗봇 시나리오 스킬 묶음」이 되는 경로**도 유효하다. 📄 37스킬 구조 = **재사용 가능한 도메인 에이전트 행동 단위**이고, 이 프로젝트의 6단계 파이프라인·`solution_rules`는 이미 그 단위로 쪼개져 있다. ⚠ 단 [[내-MCP-커넥터-환경]]·(KR-W35-68 Z.ai 코드 외부 전송) 관점에서 **사내 데이터 경계를 먼저 정해야** 하는 결정이다.
