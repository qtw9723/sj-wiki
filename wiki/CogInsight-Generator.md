...
## 진행 현황
> 📄 **현재 프로덕션 = v0.8.1**(NLU 기준 조건 설정, 2026-08-28 — PR #119 main `91628fb`, tag v0.8.1). 시나리오 **진입 조건을 `intent("<인텐트명>")`**, **발화로 갈리는 분기를 `entity("<엔티티명>:<값>")`** 로 전환. 인텐트명은 시나리오명에서 결정론 파생(한글·영문·숫자만 — `"A/S 접수"` → `AS접수`). **실봇 검증 완료** — 솔루션에 업로드해 대화로 확인, 런타임이 `intent()`·`entity()`를 정상 평가한다(v0.7.0 `match()` 미지원 같은 문제 없음). ⚠ **전제 변화: NLU 학습 데이터(CSV 2종)를 업로드하지 않으면 봇이 시나리오 진입 자체를 못 한다** — v0.8.0까지 선택이던 것이 전제가 됐다. 직전 v0.8.0(NLU 학습 데이터 생성 + CSV 다운로드, PR #118 `668e3a7`).
...
| 버전 | 내용 | 상태 |
|---|---|---|
| **v0.8.1** | NLP 기준 조건 설정 — 진입 `intent()`, 발화 분기 `entity()` | dev 검증 완료 · 프로덕션 대기 |
| **v0.9.0** | 기존 챗봇 업로드 후 수정 | 계획 |
| **v0.9.1** | 업로드 봇에서 자산 역파생(ESD·API·NLU) | 계획 |
