...
> 🔶 (2026-07-31~) **v0.6.0 dev 진행 중** — ESD 선행 생성·매핑. 2026-07-31 **아키텍처 전환**(ESD 별도 테이블 폐기 → 생성 결과 번들 `generation_tiers.esd`), 블록 A(ESD 선행 저작·검증) 완료, 블록 B+C(매핑) T1~T4 완료·T5 진행 예정. **프로덕션 무배포**. 상세는 [[#진행사항 업데이트 로그|진행 로그 2026-07-31 v0.6.0]].
...
## 진행 현황
> 📄 **현재 프로덕션 = v0.5.1**(UI/UX 앱 셸 개편 + 문서→API 생성 도구, 2026-07-29 — PR #113 main `345fa96`, tag v0.5.1). 그룹형 사이드바 앱 셸·라우팅 개편·전역 로딩 오버레이 + `/api` 문서→API 생성(신규 엣지함수 `api-doc-parse`). 직전 v0.5.0(입력 소스 다양화, PR #111 `1e3237d` — 엑셀→ESD·API문서→api_def·자연어→시나리오·POC문서→생성 4갈래 입구 + `poc-doc-parse`·`nl-scenario-parse`). DB·`solution_rules`는 v0.5.x 전 구간 무변경.
> 🔶 **현재 개발 = v0.6.0 dev**(ESD 선행 생성·매핑, 2026-07-31~ 브랜치 `feat/v0.6.0-esd-mapping`, main 미머지·프로덕션 무배포, 배지 `v0.6.0 DEV`). 아래 진행 로그 참고. 그 이전 릴리스: v0.4.2(결과 확인 창 UI, PR #108)·v0.4.1(결과 탭 사용자별, PR #107)·v0.4.0(플로우 시각화, PR #106).
...
> 📄 **v0.7.0 (계획) = 생성 결과 수정 + 문서→ESD 파싱 견고화** (2026-07-29 확정, 기존 v0.6.0 자연어 수정 흡수·확장) — 생성된 flow/`generated_json`을 세 방식으로 수정: ①자연어 지시 ②설문 기반(원설문 항목 편집→반영) ③다이얼로그 기반(대화형 부분 편집). 전체 재생성 vs 부분 패치·불변식(`checkBot`) 재검증·수정 이력(diff)은 착수 시 결정. v0.4.0 다이어그램 편집(백로그)과 상보적. **+ 블록 E(2026-07-31 추가)**: v0.6.0 코드리뷰·QA에서 드러난 **문서(엑셀)→ESD 자동생성의 조용한 오판·데이터 유실** 개선(시트 선택 부재·헤더 행 1행 고정·중복/빈 헤더 덮어쓰기·예약열 무안내 제외 등) — 근거를 `파일:라인`으로 고정해 등록.
