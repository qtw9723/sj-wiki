...
> ✅ (2026-07-28) **v0.5.0 — 입력 소스 다양화** (PR #111 머지 main `1e3237d`, tag v0.5.0 + GitHub Release). **현재 프로덕션 = v0.5.0**. 설문 폼 외 4갈래 입구(①엑셀→ESD CSV/스키마 `/convert` ②API문서(OpenAPI)→api_def ③자연어→시나리오 `/nl` ④POC문서(엑셀·PDF·MD·txt)→생성) + 신규 엣지함수 2종(`poc-doc-parse`·`nl-scenario-parse`) 배포, Vercel v0.5.0 라이브 확인. DB·`solution_rules` 무변경. ⚠ **사내 온프렘 배포·dev 리셋은 후속 미완**.
...
## 진행 현황
> 📄 **현재 프로덕션 = v0.5.0**(입력 소스 다양화, 2026-07-28 — PR #111 main `1e3237d`, tag v0.5.0). 설문 외 3개 입구 추가(엑셀→ESD, API문서→api_def, 자연어→시나리오, POC문서→생성) + 신규 엣지함수 2종(`poc-doc-parse`·`nl-scenario-parse`) 배포, Vercel 라이브 확인. DB 무변경. ⚠ **사내 온프렘 서버 배포·dev 리셋은 후속**(2026-07-28 시점 미완). 직전 v0.4.2(결과 확인 창 UI, PR #108 `2986e1f`)·v0.4.1(결과 탭 사용자별, PR #107)·v0.4.0(플로우 시각화, PR #106). 아래 진행 로그 참고.
