...
> ✅ (2026-07-15) **v0.4.1 — 결과 탭 사용자별 보기** (PR #107 머지 main `3b68501`, tag v0.4.1). **현재 프로덕션 = v0.4.1**. 결과 탭(`/results`)에 사용자 필터 드롭다운 + "사용자별로 묶어 보기" 그룹 토글. 프론트 전용(`created_by`), main 직접 릴리스(사용자 요청). ⚠ 공개 개요 페이지는 v0.4.0 기준 유지(마이너 패치라 미갱신).
...
## 진행 현황
> 📄 **현재 프로덕션 = v0.4.1**(결과 탭 사용자별 보기, 2026-07-15 — PR #107 main `3b68501`, tag v0.4.1). 직전 v0.4.0(플로우 시각화, PR #106 `52a0f59`). 둘 다 프론트 전용 → Vercel 자동 배포(DB·엣지함수 무변경). 아래 진행 로그 참고.
...
- ⚠ **공개 개요 페이지는 v0.4.0 기준 유지** — 마이너 결과탭 패치라 재배포 안 함(필요 시 버전표에 v0.4.1 행 추가 가능).
