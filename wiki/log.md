...
- 📄 사용자 확정: **v0.3.0 = 시나리오 라이브러리 테스트·고도화**(레퍼런스 0건·조립 미테스트 → 라이브러리 활용 결과 생성 테스트). [[Cogi-POC-Generator]]에 로드맵 섹션 신설 + 진행 현황·업데이트 로그 갱신, 기존 v0.3 후보는 백로그(버전 미정)로 이동.
- 저장소: `ROADMAP.md` 신설 + CLAUDE.md 포인터(브랜치 `docs/roadmap-v0.3.0`, **PR #99 — 사용자 머지 대기**). 공개 개요 페이지 v0.3.0(계획) 행 HTML 준비(**배포는 권한 차단으로 사용자 대기**).
