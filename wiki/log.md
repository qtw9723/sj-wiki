...
- 📄 "앞으로 dev 서버 세팅도 프로덕션 릴리스 규약에 추가" → 저장소 CLAUDE.md DEV 규약 갱신(PR #104, 머지 대기): 4축 후 dev 리셋·재세팅까지가 릴리스 절차, dev 슬러그 7종 main 기준 재배포, **미승격 dev-원본 레퍼런스 보존**(TRUNCATE 유실 금지), 질문 id 재발급·Storage API 삭제 주의. 메모리 규약도 동기화.
