...
- [[웹-크롤링-기초]] — RSS/API/BeautifulSoup/Playwright 4가지 방식 + AI 뉴스 수집 시스템 구축 **+ ⚠ 수집·위키화 품질 규약(2026-07-31, 8/2 갱신)**: ①롤링 피드 중복(101→60건) → URL 단위 중복제거 선행 ②자동 위키화 항목 누락 **3회**(14/28·12/28·8/30) → 수동 전수 대조 필수 ②-b 이력에 적은 건수가 본문 반영분과 불일치(12 vs 8) → 본문 grep 확인분만 기재 ③수집 공백(W29·7/29·8/1) → 파일명 날짜 연속성 확인 **③-b 파일은 있는데 소스가 통째로 빠짐(8/2 arXiv 0건)** → 소스 섹션 목록까지 확인
