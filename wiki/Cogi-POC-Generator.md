...
| **v0.3.0 (계획)** | 미정 | 로드맵 2026-07-06 (dev 진행 중) | **시나리오 레퍼런스 라이브러리 테스트·고도화** — 산업군별 레퍼런스 등록·few-shot 검증(항목 1·2 ✅), 구조 시드 생성·시드 UX 5건·피드백 스크린샷 첨부(추가 구현), 조립·결합 회귀·고도화 잔여. 전 과정 dev 전용. |
...
- ⚠ **별건 발견 (사용자 결정 대기)**: ① **`node_modules` 6,780개 파일이 git에 커밋돼 있음**(초기 커밋 잔재 — .gitignore는 기추적 파일 못 뺌). `git rm -r --cached node_modules` 정리 커밋을 할지 결정 필요(v0.3.0 머지 PR이 -6,780파일로 커질지 별도 처리할지). ② 루트에 미추적 `AGENTS.md`(CLAUDE.md 헤더 사본 형태 — 다른 도구 생성 추정, 미조치).
