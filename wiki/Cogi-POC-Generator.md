...
### 🚧 v0.2.0 설계 착수 — AI 자유응답(LLM 노드) 블록 (⚠ 설계 문서만, 미구현·미머지)
> 📄 2026-07-02 `feat/llm-response-block` 브랜치에 **설계 문서만** 추가(`docs/superpowers/specs/2026-07-02-llm-response-block-design.md`, 커밋 `d6e57d5`). **코드 0줄·main 미머지·버전 여전히 v0.1.0**. 아래는 "설계상 목표"이지 구현된 기능이 아니다.
- 📄 **목표**: 생성된 시나리오에 런타임 `llmloop` 노드(OpenAI 호출 노드)를 이용한 **LLM 자유응답 블록**을 포함시키기. 업로드된 레퍼런스(한화라이프 LLM 봇·노드 샘플)에서 파생. 접근은 기존과 동일한 **하이브리드**(규칙=DB, 강제 로직=코드).
- 📄 **v0.2.0 범위 = "단순 LLM 응답 블록"만**. 툴 정의·docsearch·`_llm.tool.name` 분기·`_llm.terminate` 루프 제어는 **범위 밖(v0.3 후보)**.
- 📄 설계된 파이프라인: 질문 2개 추가(마이그레이션만, "AI 자유응답 사용"/"주제", 질문텍스트 접두어로 결정론 감지 `collectLlmUsage()`) → Stage 2.5 `FILLABLE_TYPES`에 `llmloop` 추가 → **신규 Stage 3.7** `flow/applyLlmBlocks.ts`(메뉴 주입/폴백 교체/정규화, 결정론).
- 📄 **핵심 안전 불변식**: 생성 산출물에 `config.apiKey = ""`를 **무조건 강제** — 한화라이프 레퍼런스 `json_data`에 박힌 평문 OpenAI 키가 생성물로 새어나가지 않게 차단.
- 🧠 성격: v0.1.0의 "LLM 설계 + 코드 결정론 검증" 골격을 **LLM 응답 노드 자체까지** 확장하는 다음 스텝. 구현 착수 시 이 위키·버전표를 실제 상태로 갱신할 것.
