...
## 진행 현황 (2026-07-03 오후)
- 📄 **규모**: 총 **654 커밋** (2026-05-14 ~ 2026-07-03, `feat/llm-v0.2.0-testing` 기준, HEAD `c4b97bf`). **프로덕션 v0.1.0 / v0.2.0 ⏸ 홀드(최종 검증 통과)**. 엣지함수 26개(프로덕션 21 + dev 슬러그 5) · 마이그레이션 63개(llm 관련 **6개**는 프로덕션 미적용 — 머지 때 push) · solution_rules 8 카테고리.
...
- ⚠ **프로덕션 머지 시 주의 (📄 DB 조회로 확인, 7/3 갱신)**: llm 마이그레이션 **6개**(`20260702090000`·`090100`·`160000` + `20260703110000`·`120000`·`130000`)가 **프로덕션 테이블에 미적용**(prod questions·solution_rules에 llm 행 없음). UPDATE 계열(160000·110000·120000·130000)은 INSERT(090100) 선행 없으면 **no-op** — 머지 때 CLAUDE.md 규약대로 4축(코드/DB/엣지함수/프론트) 전체 확인 + push 후 재검증 필수. (단 `120000`의 templates 필드 추가는 7/3 라이브 반영 완료 — 멱등 가드로 중복 적용 안 됨.)
