...
> ⚠ (2026-07-03 오후 갱신) **프로덕션은 여전히 이 v0.1.0이 기준선.** v0.2.0(LLM 노드 블록)은 지식소스·시나리오별 LLM까지 구현·dev 검증을 마치고 **이 지점에서 홀드**(사용자 지시 — 추가 개발 중단, 프로덕션 머지는 별도 승인 대기) — 아래 [[#🚧 v0.2.0 — AI 자유응답(LLM 노드) 블록 (구현·dev 검증 완료 · ⏸ 홀드 · 프로덕션 미반영)|v0.2.0 블록]] 참고.
...
## 진행 현황 (2026-07-03 오후)
- 📄 **규모**: 총 **651 커밋** (2026-05-14 ~ 2026-07-03, `feat/llm-v0.2.0-testing` 기준, HEAD `9a5e96a`). **프로덕션 v0.1.0 / v0.2.0 ⏸ 홀드(최종 검증 통과)**. 엣지함수 26개(프로덕션 21 + dev 슬러그 5) · 마이그레이션 62개(llm 관련 5개는 프로덕션 미적용 — 머지 때 push) · solution_rules 8 카테고리.
- 📄 **버전 관리 시작·프로토타입 동결 (v0.1.0, 2026-06-30)**: 보고·테스트용 기준선을 SemVer 0.x로 동결(annotated tag `v0.1.0` + GitHub Release, 엣지함수 cogi-generator **v87** 배포 기준). ⚠ 동결 버전은 임의로 바꾸지 않는다(테스트는 이 태그 기준).
- 📄 **상태: v0.2.0 홀드 (2026-07-03, 사용자 지시).** 지식소스 3종·시나리오별 LLM·레퍼런스 풀 body까지 구현하고 dev API 실생성으로 검증한 상태(`3ca4282`)에서 **추가 개발을 멈추고 동결**. 프로덕션 반영(머지·배포·태깅)은 사용자 승인 시 저장소 CLAUDE.md 4축 규약대로 진행. dev 환경(dev_ 테이블·-dev 슬러그)은 이 상태 유지.
...
### 🚧 v0.2.0 — AI 자유응답(LLM 노드) 블록 (구현·dev 검증 완료 · ⏸ 홀드 · 프로덕션 미반영)
...
> 📄 **2026-07-03 홀드**: 아래 7/3 확장(생성 결함 3건 수정 + 지식소스 + 시나리오별 LLM + model 빈 값)까지 반영 후 **개발 동결**(사용자 지시). 머지 시 llm 마이그레이션 **5개**(20260702090000·090100·160000 + 20260703110000·120000)가 프로덕션 미적용 상태임에 주의.
> 📄 **머지 전 최종 검증 통과 (7/3, dev API 전 기능 매트릭스)**: 시나리오 4종(docs/general/mixed+수집·검증루프/LLM 미사용) + 전역 "둘 다"(docs)를 한 봇으로 생성해 전수 점검 — 이 과정에서 **메뉴 llmloop 주입이 시나리오 llmloop에 가려지는 결합 버그를 발견·수정**(`9a5e96a`, 시나리오 패스 선행 + 비시나리오 판정). 재생성 결과 `ebad50d0`(최종검증봇_v020, llmloop 5개: 시나리오 3+메뉴+폴백) **전 항목 통과**. 최종 홀드 지점 = **`9a5e96a`**.
...
- ⚠ **프로덕션 머지 시 주의 (📄 DB 조회로 확인, 7/3 갱신)**: llm 마이그레이션 **5개**(`20260702090000`·`090100`·`160000` + `20260703110000`·`120000`)가 **프로덕션 테이블에 미적용**(prod questions·solution_rules에 llm 행 없음). UPDATE 계열(160000·110000·120000)은 INSERT(090100) 선행 없으면 **no-op** — 머지 때 CLAUDE.md 규약대로 4축(코드/DB/엣지함수/프론트) 전체 확인 + push 후 재검증 필수. (단 `120000`의 templates 필드 추가는 7/3 라이브 반영 완료 — 멱등 가드로 중복 적용 안 됨.)
