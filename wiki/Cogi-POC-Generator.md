...
> ⚠ (2026-07-03 갱신) **프로덕션은 여전히 이 v0.1.0이 기준선.** v0.2.0(LLM 노드 블록)은 구현됐지만 main에서 롤백돼 **dev 환경에서 테스트 중** — 아래 [[#🚧 v0.2.0 — AI 자유응답(LLM 노드) 블록 (구현 완료 · dev 테스트 중 · 프로덕션 미반영)|v0.2.0 블록]] 참고.
...
## 진행 현황 (2026-07-03)
- 📄 **규모**: 총 **646 커밋** (2026-05-14 ~ 2026-07-03, `feat/llm-v0.2.0-testing` 기준). **프로덕션 v0.1.0 / v0.2.0 dev 테스트 중**. 엣지함수 26개(프로덕션 21 + dev 슬러그 5) · 마이그레이션 60개 · solution_rules 8 카테고리(dev 30행 · prod 28행).
...
- 🧠 **상태: 프로덕션 v0.1.0 운영 + v0.2.0(LLM 노드 블록) dev 검증 체제.** 7/2 v0.2.0이 main까지 갔다가 롤백된 뒤, **dev 환경(dev_ 테이블·-dev 슬러그)에서 실행-후-머지 방식으로 검증 중**. 작업 규약(dev-only 작업 / 머지 시 전체 마이그레이션 / 머지 후 dev 리셋)이 저장소 CLAUDE.md로 명문화됨(7/3).
...
### 🚧 v0.2.0 — AI 자유응답(LLM 노드) 블록 (구현 완료 · dev 테스트 중 · 프로덕션 미반영)
> 📄 2026-07-02 하루에 설계→구현→main 머지·릴리스(PR #94~#96)까지 갔다가 **같은 날 PR #97로 전량 롤백**. 프로덕션은 v0.1.0 유지, v0.2.0은 CHANGELOG상 **"미릴리스(dev 테스트 중)"** — v0.2.0 태그 없음. 이후 작업은 `feat/llm-v0.2.0-testing` 브랜치 + dev 환경에 누적 중이며, 프로덕션 반영(머지·배포·태깅)은 사용자 승인 시점에 진행.
...
- ⚠ **프로덕션 머지 시 주의 (📄 DB 조회로 확인)**: llm 마이그레이션 3개(`20260702090000`·`090100`·`160000`)가 **프로덕션 테이블에 미적용**(prod questions·solution_rules에 llm 행 없음). 특히 `160000`은 UPDATE라 INSERT(090100) 선행 없으면 **no-op** — 머지 때 CLAUDE.md 규약대로 4축(코드/DB/엣지함수/프론트) 전체 확인 + push 후 재검증 필수.
