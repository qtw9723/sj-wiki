...
> 📄 저장소에 **`ROADMAP.md` 신설**(+CLAUDE.md 상단 포인터) — 브랜치 `docs/roadmap-v0.3.0`, **PR #99(머지 대기)**. 머지 후 저장소 ROADMAP.md가 계획의 SoT.
...
## 진행사항 업데이트 로그
### 2026-07-06 — v0.3.0 로드맵 수립(시나리오 라이브러리) + ROADMAP.md 신설 (📄 사용자 지시)
...
- 저장소에 **ROADMAP.md 신설 + CLAUDE.md 포인터** — 브랜치 `docs/roadmap-v0.3.0`, **PR #99 생성(머지는 사용자 대기)**. 기존 v0.3 후보는 백로그(버전 미정)로 이동.
- 공개 개요 페이지: 버전 히스토리에 "v0.3.0 (계획)" 로드맵 행을 넣은 HTML을 준비했으나 **⚠ 배포 미완** — `vercel --prod`·PR 머지가 권한 정책으로 차단돼 사용자 실행 대기(준비본: 세션 스크래치패드 `cogi-overview/index.html`+`favicon.svg`). 아티팩트 동기화도 배포와 함께 진행 예정.
