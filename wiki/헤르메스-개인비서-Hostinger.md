...
- ⭐ 🆕 **(2026-08-07) 이 구상의 평가 기준이 논문으로 나왔다 — MemArena** ([[AI-주간-소식-2026-W32]] 10차 보강): 📄 arXiv **MemArena: An Ego-Centric Benchmark for On-Device Agentic Personal Memory Assistants at Scale**([2608.02613](https://arxiv.org/abs/2608.02613)) — *엣지에 배치된 개인 메모리 어시스턴트는 **사적인 대인 대화를 온디바이스에서 오픈웨이트 모델로** 처리해야 한다. 그러나 기존 메모리 벤치마크는 **활동 밀집 상호작용 + 자기중심(ego-centric) 관점 + 대규모**의 조합을 충분히 시험하지 않는다.*
  - 🧠 §7-1 멀티 페르소나·§7-2 자가개선 루프가 전제하는 조건(사적 대화 + 내 관점 + 자체 호스팅)이 **그대로 논문의 전제**다. 즉 이 구상은 취향이 아니라 **연구 커뮤니티가 지금 벤치마크를 만드는 중인 문제**다 → [[프로젝트-포트폴리오]] 서술에 쓸 수 있다.
  - 🧠 같은 주 📄 **LFM2.5-2.6B**("로컬 에이전트를 어디에나 배포", [[AI-주간-소식-2026-W32]] 6차 보강)와 짝이다 — **모델과 평가 기준이 같은 주에 도착**했다. VPS 자체 호스팅 노선이 "차선책"이 아니라 **의도된 배치 형태**가 되는 배경.
  - ⚠ 🧠 그리고 W32 연결점 ⑬(📄 MAPLE-Guard — *오염된 메모리는 한 번만 써 넣으면 되고 이후 지속 작동한다*)이 개인 데이터에서 더 무겁다 — **개인비서 메모리는 틀려도 대조할 원본이 없다**(대화는 재생되지 않는다). 🧠 이 위키가 `raw/`를 보존하는 것과 같은 논리로, **원본 대화 로그 보존 + 메모리 쓰기 게이트**를 설계에 넣어야 한다(🧠 제안, 미구축).
...
- [[AI-주간-소식-2026-W32]] — 🆕 ⭐ **MemArena = 온디바이스 개인 메모리 어시스턴트의 첫 평가 기준**(10차 보강) · **LFM2.5-2.6B 로컬 에이전트 배포**(6차 보강) · ⚠ **MAPLE-Guard = 메모리 쓰기 게이트 필요**(연결점 ⑬) · **OneDayAgent = 업무·학습·생활 교차 롱호라이즌 벤치마크**(11차 보강)
- 작성 예정: [[claude-api]]
