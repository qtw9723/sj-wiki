...
- 🆕 🔥 🚨 ⭐⭐ **(2026-09-04) 「일정을 읽어 다른 서비스로 흘린다」가 상용 플랫폼의 공식 로드맵이 됐다 — 검증 + 희소성 감소 동시에** (📄 [[AI-주간-소식-2026-W36]] 9차 감사 (KR-W36-77)):
  - 📄 **타임트리**(창립 12주년)가 **SCE(Smart Calendar Engine)** 기반으로 *"AI 에이전트가 일정 정보를 다른 서비스와 연계"* 하며, 향후 **"다양한 서비스·기기에서 작동하는 AI 에이전트 인프라"** 로 제공 예정([스마트투데이](https://www.smarttoday.co.kr/ko-kr/articles/111144)). 📄 같은 파일 (KR-W36-74) **LG전자 씽큐 클로**도 *"텍스트 채팅으로 생활 맥락을 파악해 가전·일정 서비스를 연결"* 한다.
  - 🧠 **이 vault에는 같은 문제를 푸는 프로젝트가 넷 있다**: [[schedule-reporter-kakao]](스케줄 → 리포트) · [[Teams-Gmail-캘린더-Gemini-연동]](캘린더 ↔ 메일 ↔ LLM) · [[모닝-브리핑-AppsScript]](일정 → 아침 브리핑) · 그리고 이 문서(always-on 스케줄러 + 외부 연동). → **문제 선택이 틀리지 않았다는 외부 확인**이다.
  - 🚨 🧠 **동시에 희소성은 준다** — 📄 (KR-W36-28) 천안시 공무원 500명 바이브 코딩 · (KR-W36-63) 부산대 20개 부서 Claude Team에서 얻은 결론과 같다: **도구를 쓰는 것이 아니라 설계·판단이 차별점**이다. ✅ 그러므로 이 문서 §7 구상의 차별화 문장은 *"일정을 연동한다"* 가 아니라 **"교회·업무·개인이 섞인 내 맥락에서 이 일정을 어디로 흘릴지 판단하고, 그 판단을 나중에 설명할 수 있다"** 쪽에 둔다.
  - ✅ 🧠 **구체적 함의 — 「연결」은 이미 포화, 값은 「관측·증거」에 있다**: 📄 [[에이전트-자동화-도구]] §관제 절이 정리한 3층(**연결 → 관측 → 증거**)에서 연결 층에는 이제 통신사(KT)·가전(LG)·플랫폼(타임트리)이 다 들어왔다. 🧠 개인 비서에서 남는 차별점은 **"왜 그 일정을 그렇게 처리했는지"를 사후에 보여 주는 것**이며, 이는 위 §7-2 자가개선 루프와 §의외의연결점 MemArena 항목(원본 대화 로그 보존 + 메모리 쓰기 게이트)이 이미 가리키던 방향과 **같다**.
...
- 🆕 🔥 🚨 [[AI-주간-소식-2026-W36]] — **(KR-W36-77) 타임트리 SCE = 「캘린더 → AI 에이전트 인프라」**(내 프로젝트군과 같은 문제 정의) · (KR-W36-74) LG 씽큐 클로 · 🧠 **연결 층 포화 → 값은 관측·증거 층**
- 🆕 ⭐⭐ [[AI-주간-소식-2026-W34]] · [[AI-주간-소식-2026-W33]] **(2026-08-18 보강) — 개인 비서의 시간 단위가 「1년」이 됐고, 같은 주에 「축적의 부패」를 말하는 논문이 둘 왔다**: 📄 **MobileMem**(`2608.13606`) — *다음 세대 에이전트는 고립된 질문 응답을 넘어 **사용자 경험을 이해·기억하고 지속 학습하는 영속적 개인 비서**로 이동 중이며, 장기 메모리가 필요하다*. ⚠ 반대편 둘 — 📄 **(W33-EN42) Beyond Memory: Transactional Continuity Kernel**(`2608.11632`, ***보존만으로는 권위 있는 상태를 식별하지 못한다*** · 컨트롤 플레인이 없으면 **stale overwrite**) · 📄 **(W33-EN45) EvoGraph-Mem**(`2608.11248`, ***저장된 메모리의 품질이 시간에 따라 열화한다*** → 실패 인식 **편집 가능** 그래프 메모리). ✅ 설계 함의: 장기 메모리에는 저장·검색 외에 **「무엇이 지금 진짜인가」와 「무엇이 어떤 실패로 폐기됐는가」** 를 가리키는 층이 따로 필요하다. 🆕 📄 **(W33-EN50)** *동작하는 스크립트 · 신뢰받는 프로토콜 · **실패에 붙은 경고** · 새 질문을 옛 결과에 잇는 판단*(`2608.11224`) = 그 층의 내용물 목록.
